...
fn multiple_combining_characters() {
    ...
    if !created.is_empty() {
        ...
        assert_eq!(paths.len(), created.len(), "should find all created files");
    }
}
...
fn sorting_by_byte_length() {
    ...
    for i in 0..paths.len().saturating_sub(1) {
        assert!(paths[i] < paths[i + 1], "paths should be sorted by bytes");
    }
}
...
fn deeply_nested_unicode_path() {
    ...
    assert!(
        paths
            .iter()
            .any(|p| p.to_str().is_some_and(|s| s.ends_with("file.txt"))),
