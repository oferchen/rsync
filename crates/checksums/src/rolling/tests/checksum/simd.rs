...
#[test]
fn avx512_accumulate_matches_scalar_reference() {
    if !std::arch::is_x86_feature_detected!("avx512bw") {
        return;
    }

    use crate::rolling::checksum::accumulate_chunk_scalar_for_tests;
    use crate::rolling::checksum::x86::accumulate_chunk_avx512_for_tests;

    let sizes = [64usize, 65, 95, 128, 191, 256, 1024, 4096, 8192];
    let seeds = [
        (0u32, 0u32, 0usize),
        (0x1234u32, 0x5678u32, 7usize),
        (0x0fffu32, 0x7fffu32, 1024usize),
        (0xffffu32, 0xffffu32, usize::MAX - 128),
    ];
    ...
    for &(seed_s1, seed_s2, seed_len) in &seeds {
        for &size in &sizes {
            ...
            let scalar = accumulate_chunk_scalar_for_tests(seed_s1, seed_s2, seed_len, &data);
            let simd = accumulate_chunk_avx512_for_tests(seed_s1, seed_s2, seed_len, &data);

            assert_eq!(
                scalar, simd,
                "AVX-512 mismatch for size {size} with seeds {seed_s1:#x}/{seed_s2:#x}/{seed_len}",
            );
        }
    }
}
