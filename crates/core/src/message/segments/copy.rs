...
/// Error returned when [`MessageSegments::copy_to_slice`] receives an
/// undersized buffer.
...
impl CopyToSliceError {
    ...
    /// Returns how many additional bytes were necessary to satisfy the copy
    /// operation.
    ///
    /// The calculation uses saturating subtraction so that callers can report
    /// the missing capacity directly in diagnostics even if the error was
    /// created with inconsistent inputs. When the error originates from
    /// [`MessageSegments::copy_to_slice`], the result matches `required -
    /// provided`, mirroring upstream rsync's messaging when a scratch buffer is
    /// too small to hold the fully rendered diagnostic.
    #[must_use]
...
impl<'a> MessageSegments<'a> {
    ...
    ///
    /// The destination slice must be at least [`Self::len`] bytes long. When the
    /// capacity is insufficient the method returns [`CopyToSliceError`]
    /// describing the required length so callers can retry with a suitably sized
    /// buffer. This mirrors upstream rsync's approach of reusing stack-allocated
    /// buffers for message rendering while preserving deterministic allocation
    /// patterns.
    ///
    /// # Examples
    ///
