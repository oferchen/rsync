...
fn write_borrowed_sequential<'a, W: IoWrite>(
    writer: &mut W,
    slices: &[IoSlice<'a>],
    mut remaining: usize,
) -> io::Result<()> {
...
fn trim_leading_empty_slices_mut<'a>(mut slices: &'a mut [IoSlice<'a>]) -> &'a mut [IoSlice<'a>] {
    loop {
...
fn trim_leading_empty_slices<'a>(mut slices: &'a [IoSlice<'a>]) -> &'a [IoSlice<'a>] {
