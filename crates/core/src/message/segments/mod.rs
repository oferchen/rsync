...
use super::MAX_MESSAGE_SEGMENTS;
...
pub use copy::CopyToSliceError;

/// Collection of slices that jointly render an [`Message`].
///
/// The segments reference the message payload together with optional exit codes,
/// source locations, and role trailers. Callers obtain the structure through
/// [`Message::as_segments`] and can then stream the slices into vectored
/// writers, aggregate statistics, or reuse the layout when constructing custom
/// buffers. `MessageSegments` implements [`AsRef`] so the collected [`IoSlice`]
/// values can be passed directly to APIs such as
/// [`write_vectored`](std::io::Write::write_vectored) without requiring an
/// intermediate allocation.
///
/// # Examples
///
/// Convert the segments into a slice suitable for
...
/// ```
///
/// Consume the segments to collect the rendered message into a contiguous
/// buffer.
///
/// ```
/// use rsync_core::{
...
impl<'a> MessageSegments<'a> {
    ...
    ///
    /// The iterator traverses the same slices that [`Self::as_slices`] exposes,
    /// preserving their original ordering so call sites can stream the message
    /// into custom sinks without allocating intermediate buffers. This mirrors
    /// upstream rsync's behaviour where formatted messages are emitted
    /// sequentially. The iterator borrows the segments, meaning the caller must
    /// keep the [`MessageSegments`] instance alive for the duration of the
    /// iteration.
    ///
    /// # Examples
    ///
    ...
    ///
    /// This is a convenience wrapper around [`Self::iter`] that exposes the
    /// underlying `&[u8]` views directly. It is especially useful when callers
    /// need to analyse or copy the rendered bytes without interacting with
    /// [`IoSlice`] explicitly, keeping their code agnostic of vectored I/O
    /// details. The iterator preserves the original ordering and implements
    /// [`DoubleEndedIterator`], [`ExactSizeIterator`], and [`FusedIterator`] so
    /// integrations can efficiently consume the slices in either direction.
    ///
    /// # Examples
    ///
    /// ```
