...
use crate::client::{
    ClientError, DaemonAddress, FEATURE_UNAVAILABLE_EXIT_CODE, Role, daemon_error,
};
...
pub(super) fn load_daemon_connect_program(
...
pub(super) fn spawn_connect_program(
    addr: &DaemonAddress,
    program: &ConnectProgramConfig,
) -> Result<ConnectProgramStream, ClientError> {
    let command = program
    ...
    Ok(ConnectProgramStream::new(child, stdin, stdout))
}

pub(super) struct ConnectProgramStream {
    child: Child,
    stdin: ChildStdin,
    stdout: ChildStdout,
}
