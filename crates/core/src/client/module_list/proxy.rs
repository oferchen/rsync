...
use super::address::{SocketAddrDisplay, decode_host_component, hex_value, parse_bracketed_host};
use crate::client::{ClientError, DaemonAddress, Role, SOCKET_IO_EXIT_CODE, socket_error};
...
impl ProxyConfig {
    pub(super) fn display(&self) -> SocketAddrDisplay<'_> {
        SocketAddrDisplay {
            host: &self.host,
            port: self.port,
        }
...
pub(super) fn connect_via_proxy(
    ...
    for candidate in addrs {
        match super::connect::connect_with_optional_bind(candidate, bind_address, connect_timeout) {
            Ok(stream) => {
...
pub(super) fn proxy_response_error(text: impl Into<String>) -> ClientError {
    let message =
