...
impl BandwidthLimit {
    ...
    pub const fn from_components(components: bandwidth::BandwidthLimitComponents) -> Option<Self> {
        match components.rate() {
            Some(rate) => Some(Self::from_bytes_per_second(rate)),
            None => None,
        }
    }
    ...
    /// for this limit.
    ///
    /// Returning a dedicated value keeps the conversion explicit while avoiding
    /// the need for callers to reach into the `bandwidth` crate directly when
    /// they already hold a [`BandwidthLimit`].
    #[must_use]
    pub const fn components(&self) -> bandwidth::BandwidthLimitComponents {
        bandwidth::BandwidthLimitComponents::new(Some(self.bytes_per_second))
    }
    ...
    ///
    /// Returning a concrete limiter keeps higher layers from re-encoding the
    /// rate when they need to apply throttling to local copies or daemon
    /// transfers.
    #[must_use]
    pub fn to_limiter(&self) -> BandwidthLimiter {
        BandwidthLimiter::new(self.bytes_per_second)
    }
...
mod tests {
    ...
    mod bandwidth_limit_tests {
        ...
        fn from_components_with_rate() {
            let components =
                bandwidth::BandwidthLimitComponents::new(Some(NonZeroU64::new(512).unwrap()));
        ...
        fn from_components_without_rate() {
            let components = bandwidth::BandwidthLimitComponents::new(None);
            let limit = BandwidthLimit::from_components(components);
        ...
        #[test]
        fn parse_rejects_burst_component() {
            // upstream: the client --bwlimit is a bare size; no `:BURST`.
            assert!(BandwidthLimit::parse("100:50").is_err());
        }
