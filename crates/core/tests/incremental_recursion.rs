...
fn deep_directory_tree_transfers_all_levels() {
    ...
    // Level 0: src/root.txt
    // Level 1: src/a/a.txt
    // Level 2: src/a/b/b.txt
    // Level 3: src/a/b/c/c.txt
    // Level 4: src/a/b/c/d/d.txt
    // Level 5: src/a/b/c/d/e/e.txt
...
fn empty_directories_are_preserved() {
    ...
    // Sibling of a populated dir is also empty.
    fs::create_dir_all(source.join("deep/path/sibling_empty")).expect("sibling empty");
    // Place a file alongside to ensure the parent is not pruned.
    ...
    // Verify all empty directories exist in the destination.
    assert!(
    ...
    // Populated content is also present.
    assert_eq!(
...
fn symlinks_within_incremental_directories() {
    ...
    // Create a tree where symlinks appear at various depths.
    ...
    // Symlinks recreated with correct targets.
    ...
    // Symlinks count toward the total.
    assert!(
...
fn large_file_count_single_directory() {
    ...
    // Create 500 files with distinct sizes to avoid quick-check collisions.
    ...
    // Add a few files outside the large directory to ensure the rest of the
    ...
    // All 500 + 2 regular files should be transferred.
    ...
    // Verify content integrity for a sample.
    ...
    // Non-large-dir files also present.
...
fn mixed_content_deep_tree() {
    ...
    // Level 3: another symlink and file.
    touch(
    ...
    // Level 4: deeply nested empty directory.
...
fn second_pass_skips_unchanged_files() {
    ...
    // Backdate destination files to 2 seconds before source so quick-check
    // uses mtime comparison rather than being confused by same-second writes.
    // The source mtime is already set, so we just need the dest to have the
    // same mtime - which it should after -t. But verify content is present.
    assert_eq!(
        fs::read(dest.join("a/file1.txt")).unwrap(),
        b"content-one-xxx"
    );

    // Second pass - nothing changed, files should be skipped.
