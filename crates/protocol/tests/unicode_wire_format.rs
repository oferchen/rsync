...
fn roundtrip_entry(entry: &FileEntry, protocol: ProtocolVersion) -> FileEntry {
    ...
    reader.read_entry(&mut cursor).expect("read failed").expect("no entry")
}
...
fn wire_format_latin_extended() {
    ...
    for protocol in [ProtocolVersion::V28, ProtocolVersion::V30, ProtocolVersion::NEWEST] {
        for name in &names {
            ...
            assert_eq!(decoded.name(), *name, "Mismatch for {} with {:?}", name, protocol);
        }
    }
}
...
fn wire_format_cjk_nested_directories() {
    ...
    assert_eq!(decoded[2].name(), "\u{65e5}\u{672c}/\u{4e2d}\u{56fd}/\u{d55c}\u{ad6d}.txt");
}
...
fn wire_format_mixed_scripts_nested_paths() {
    ...
    let decoded = roundtrip_entries(&entries, ProtocolVersion::NEWEST);
    assert_eq!(decoded.len(), 3);
    assert_eq!(decoded[2].name(), format!("{}/{}/{}", latin_dir, japanese_dir, arabic_file));
}
...
fn wire_format_deeply_nested_unicode() {
    ...
    assert_eq!(decoded[3].name(), "\u{4e2d}\u{6587}/\u{65e5}\u{672c}\u{8a9e}/\u{d55c}\u{ad6d}\u{c5b4}/file.txt");
}
...
fn wire_format_long_unicode_paths() {
    ...
    let path = format!("{}/{}/{}.txt", dir1, dir2, file);
...
fn wire_format_prefix_compression_unicode() {
    ...
    let entries: Vec<_> = (0..10)
        .map(|i| test_file(&format!("{}file_{:03}.txt", prefix, i)))
        .collect();
    ...
    assert!(buf.len() < 300, "Expected better compression, got {} bytes", buf.len());
...
fn wire_format_unicode_symlinks() {
    ...
    let mut buf = Vec::new();
    writer.write_entry(&mut buf, &symlink1).expect("write failed");
    writer.write_end(&mut buf, None).expect("write end failed");
    ...
    writer2.write_entry(&mut buf2, &symlink2).expect("write failed");
    writer2.write_end(&mut buf2, None).expect("write end failed");

    let mut cursor2 = Cursor::new(&buf2);
    let mut reader2 = FileListReader::new(protocol).with_preserve_links(true);
    let decoded2 = reader2.read_entry(&mut cursor2).expect("read failed").expect("no entry");
    assert_eq!(decoded2.link_target().expect("should have target").to_str().unwrap(),
...
fn wire_format_unicode_stress_many_entries() {
    ...
    let entries: Vec<_> = (0..1000)
        .map(|i| {
            ...
            test_file(&format!("{}_file_{:04}.txt", prefix, i))
        })
        .collect();
