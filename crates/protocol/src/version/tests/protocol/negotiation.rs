...
use crate::version::{
    ProtocolVersion, MAXIMUM_PROTOCOL_ADVERTISEMENT, SUPPORTED_PROTOCOL_BOUNDS,
...
fn negotiation_succeeds_for_all_supported_versions() {
    for version in 28..=32 {
        ...
        assert!(
            result.is_ok(),
            "version {version} should be negotiable"
        );
        assert_eq!(
            result.unwrap().as_u8(),
            version,
...
fn version_28_feature_profile() {
    ...
    assert!(v.uses_legacy_ascii_negotiation(), "v28 uses legacy negotiation");
    ...
    assert!(!v.safe_file_list_always_enabled(), "v28 lacks always-on safe file list");
}
...
fn version_29_feature_profile() {
    ...
    assert!(v.uses_legacy_ascii_negotiation(), "v29 uses legacy negotiation");
    ...
    assert!(!v.safe_file_list_always_enabled(), "v29 lacks always-on safe file list");
}
...
fn version_30_feature_profile() {
    ...
    assert!(!v.uses_legacy_ascii_negotiation(), "v30 does not use legacy negotiation");
    ...
    assert!(!v.safe_file_list_always_enabled(), "v30 lacks always-on safe file list");
}
...
fn version_31_feature_profile() {
    ...
    assert!(!v.uses_legacy_ascii_negotiation(), "v31 does not use legacy negotiation");
    assert!(v.uses_binary_negotiation(), "v31 uses binary negotiation");
    assert!(v.supports_sender_receiver_modifiers(), "v31 has s/r modifiers");
    assert!(v.supports_perishable_modifier(), "v31 has perishable modifier");
    ...
    assert!(v.safe_file_list_always_enabled(), "v31 adds always-on safe file list");
}
...
fn version_32_feature_profile() {
    ...
    assert!(!v.uses_legacy_ascii_negotiation(), "v32 does not use legacy negotiation");
    ...
    assert!(v.safe_file_list_always_enabled(), "v32 has always-on safe file list");
}
...
fn no_mutual_protocol_error_includes_peer_versions() {
    ...
    assert!(msg.contains("we support"), "error should mention our support");
}
