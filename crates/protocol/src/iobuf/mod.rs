...
//! (32 KiB, io.c:1427) and the output buffer is twice that (64 KiB, io.c:1408).
//! The comments at io.c:597 (`"We never resize the circular input buffer."`)
//! and io.c:612 (the same for the output buffer) are load-bearing: a request
...
//!   buffer when multiplexing starts (io.c:2499-2500) and **back-filled at
//!   flush time** (io.c:705-706). Header and payload are therefore adjacent
...
//!   its payload.
//! * Appending bytes that fit performs **no I/O at all** (io.c:2293-2322):
...
//! (io.c:856-895). [`OutBuf`] reproduces exactly that: the drain loop is
...
/// (io.c:136-143 `IOBUF_WAS_REDUCED` / `IOBUF_RESTORE_SIZE`).
