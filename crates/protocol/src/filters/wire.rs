...
mod tests {
    ...
    /// Risk rules with `receiver_side: true` must serialize as `R *.log`,
    /// not `Rr *.log`. Upstream's parser rejects an explicit `r` modifier
    /// when the prefix already implies the side (exclude.c:1180-1278).
    /// See PR #3464.
    #[test]
    fn risk_with_receiver_side_serializes_without_r_modifier() {
        let protocol = ProtocolVersion::from_supported(32).unwrap();
        ...
        let bytes = serialize_rule(&rule, protocol).unwrap();
        // Wire payload should be exactly "R *.log" - no redundant `r` flag.
        assert_eq!(bytes, b"R *.log");
    }

    #[test]
    fn protect_with_receiver_side_serializes_without_r_modifier() {
        let protocol = ProtocolVersion::from_supported(32).unwrap();
        let rule = FilterRuleWireFormat {
        ...
        let bytes = serialize_rule(&rule, protocol).unwrap();
        assert_eq!(bytes, b"P *.log");
    }

    /// End-to-end roundtrip: write a Risk rule with `receiver_side: true`,
    /// read it back. The wire form is `R *.log` and the parser sees
    /// `RuleType::Risk` (the `R` prefix already implies receiver side).
    #[test]
    fn risk_filter_roundtrip_v32() {
        let protocol = ProtocolVersion::from_supported(32).unwrap();
        let rule = FilterRuleWireFormat {
            rule_type: RuleType::Risk,
            pattern: "*.log".to_owned(),
        ...
        let parsed = read_filter_list(&mut &buf[..], protocol).unwrap();
        assert_eq!(parsed.len(), 1);
        assert_eq!(parsed[0].rule_type, RuleType::Risk);
        assert_eq!(parsed[0].pattern, "*.log");
    }
