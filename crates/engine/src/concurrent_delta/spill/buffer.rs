//! In-memory ring buffer with disk-spill overflow for the reorder layer.
//!
//! [`SpillableReorderBuffer`] wraps a [`ReorderBuffer`] and pushes excess
//! items to a temporary file when the in-memory footprint exceeds the
//! configured threshold. See [`super`] for the spill layer's overall
//! design, codec contract, and error semantics.
...
use super::{SpillBackend, SpillCodec, SpillError, SpillStats, open_backend};
...
impl<T: SpillCodec> SpillableReorderBuffer<T> {
    ...
    pub fn with_default_threshold(capacity: usize) -> Self {
        Self::new(capacity, super::DEFAULT_SPILL_THRESHOLD)
    }
