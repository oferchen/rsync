...
//!
//! This submodule houses the full buffer surface: the struct definition, its
//! [`Debug`] impl, every public method, the private spill/reload helpers, and
//! the buffer-focused test suite. The parent [`spill`](super) module re-exports
//! [`SpillableReorderBuffer`] so the canonical public path
//! (`engine::concurrent_delta::spill::SpillableReorderBuffer`) stays stable.
//!
//! Constants that are only consumed by the buffer surface
//! ([`HOT_ZONE`], [`SPILL_TAG_RAW`], [`SPILL_TAG_ZSTD`]) live here. The
//! public [`DEFAULT_SPILL_THRESHOLD`](super::DEFAULT_SPILL_THRESHOLD) and the
//! [`SpillCodec`](super::SpillCodec) trait stay in [`spill`](super) so the
//! cross-cutting symbols remain at their documented anchor.
...
use super::tempfile::{SpillBackend, open_backend};
use super::{
    DEFAULT_SPILL_THRESHOLD, SpillCodec, SpillCompression, SpillError, SpillGranularity,
    SpillReclaim, SpillStats, rss,
};
