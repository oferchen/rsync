...
fn execute_respects_exclude_filter() {
    ...
    let filters = FilterSet::from_rules([oc_rsync_filters::FilterRule::exclude("*.tmp")])
...
fn execute_prunes_empty_directories_when_enabled() {
    ...
    let filters_without = FilterSet::from_rules([oc_rsync_filters::FilterRule::exclude("*.tmp")])
        .expect("compile filters");
    ...
    let filters_with = FilterSet::from_rules([oc_rsync_filters::FilterRule::exclude("*.tmp")])
        .expect("compile filters");
...
fn execute_respects_include_filter_override() {
    ...
    let filters = FilterSet::from_rules([
        oc_rsync_filters::FilterRule::exclude("*.tmp"),
        oc_rsync_filters::FilterRule::include("keep.tmp"),
    ])
    .expect("compile filters");
    let options = LocalCopyOptions::default().filters(Some(filters));
...
fn dir_merge_clear_keyword_discards_previous_rules() {
    ...
    assert!(entries.rules.iter().any(|rule| {
        rule.pattern() == "skip" && matches!(rule.action(), oc_rsync_filters::FilterAction::Exclude)
    }));
