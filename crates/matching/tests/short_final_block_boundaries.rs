...
//!
//! `generator.rs` already pins the main behaviours in-module: the tail is
//! matched, it is emitted as a standalone `Copy`, and a mid-file stripe does not
//! probe it. This file covers the edges of that handling, which have a
//! documented history of going wrong.
//!
...
/// A basis that is nothing but one short block - no full block at all.
///
/// This is the degenerate end of the same range, and it is also the case that
/// made the index constructor refuse the signature outright.
#[test]
fn a_basis_of_only_one_short_block_is_matched_end_to_end() {
    let basis = pattern(BLOCK_LEN as usize - 3);
    let index = build_index(&basis, BLOCK_LEN);
    assert_eq!(index.block_count(), 1);
    assert!(index.block(0).len() < index.block_length());

    let script = scan(&basis, &index);
    assert_eq!(copy_lens(&script), vec![basis.len()]);
    assert_eq!(script.literal_bytes(), 0);
    assert_eq!(reconstruct(&script, &basis, &index), basis);
}
...
/// The consecutive-match extension deliberately leaves the tail as a literal.
///
/// That path halves the per-block strong sum and only trusts a match a
/// neighbouring block corroborates; the final short block has no successor, so
/// admitting it would add a second, weaker acceptance rule on the one path
/// running half-length checksums. The result stays byte-exact - only larger -
/// and the path is opt-in on both peers (`OC_CONSECUTIVE_MATCH`), so the default
/// wire is unaffected. Pinned so the choice cannot drift silently.
#[test]
fn the_consecutive_match_gate_leaves_the_tail_literal() {
    let tail_len = 7usize;
    let basis = pattern(BLOCK_LEN as usize * 6 + tail_len);
    let index = build_index(&basis, BLOCK_LEN);
