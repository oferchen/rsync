...
pub mod cancel {
    ...
    pub fn cancel_by_user_data(
        _ring: &mut StubIoUring,
        _user_data: u64,
...
/// Stub linked-chain primitive (not available on this platform).
///
/// Mirrors the Linux [`crate::io_uring::linked_chain`] surface so cross-
/// platform callers can name the types without `cfg`-gating. The chain
/// cannot be constructed because the stub [`RingLease`] cannot exist on
/// this platform.
