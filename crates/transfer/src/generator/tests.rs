...
/// Regression: a subdirectory deleted between `read_dir()` and the subsequent
/// stat must be reported via `IOERR_VANISHED` and not abort `build_file_list`.
///
/// upstream: flist.c send_directory / interpret_stat_error - ENOENT during the
/// recursive walk emits a warning, sets `IOERR_VANISHED`, and continues.
#[cfg(unix)]
#[test]
fn build_file_list_tolerates_vanished_subdirectory() {
    let temp_dir = TempDir::new().unwrap();
    let base = temp_dir.path();
    fs::write(base.join("survivor.txt"), b"ok").unwrap();
    let doomed = base.join("doomed");
    fs::create_dir(&doomed).unwrap();
    fs::write(doomed.join("inner.txt"), b"bye").unwrap();
    // Remove the subdir to simulate a race between readdir() and stat().
    fs::remove_dir_all(&doomed).unwrap();

    let (_handshake, mut ctx) = test_generator_for_path(base, true);
    let count = ctx.build_file_list(&[base.to_path_buf()]).unwrap();

    assert!(count >= 2, "survivor entries must remain after vanish");
    let names: Vec<String> = ctx
        .file_list()
        .iter()
        ...
        .collect();
    assert!(
        names.iter().any(|n| n.ends_with("survivor.txt")),
        "survivor.txt missing from list: {names:?}"
    );
    assert!(
        !names.iter().any(|n| n.contains("doomed")),
        "vanished subdir must not appear: {names:?}"
    );
    assert!(
        ctx.io_error() & io_error_flags::IOERR_VANISHED != 0,
        "IOERR_VANISHED must be set after a vanished entry, got {:#x}",
        ctx.io_error()
    );
}
