...
//! - `flist.c:2636-2651` - `send_extra_file_list()` lookahead loop head
//! - `sender.c:516,550` - send loop tops the window up to the minimum
//! - `io.c:854-875` - `perform_io()` grows the window towards the maximum
...
/// - `sender.c:516,550` - `send_extra_file_list(f_out, MIN_FILECNT_LOOKAHEAD)`
...
/// - `io.c:854-861` - `perform_io()` keeps `extra_flist_sending_enabled` set,
...
/// # Upstream Reference
///
/// - `flist.c:2636` - `send_extra_file_list()` sends one directory's entries
///   as a sub-list
...
pub(crate) struct PendingSegment {
    ...
    /// (flist.c:2388) to identify which directory this sub-list belongs to.
...
/// `sender.c:516,550`), again at the point it is about to block for receiver
/// input (upstream `io.c:873`), and tells it when the receiver has finished a
...
/// (`flist.c:2651`), where `file_total` counts entries in every live file-list
...
/// sub-list adds its entry count (`flist.c:2707` `file_total += flist->used`,
...
/// place (`sender.c:532-536`).
///
/// # Upstream Reference
///
/// - `sender.c:516,550` - checks `inc_recurse` at top/bottom of send loop
/// - `flist.c:2651` - `send_extra_file_list()` re-tests the lookahead
///   condition on every iteration
/// - `io.c:854-875` - the idle-time growth path and its ceiling
#[derive(Debug)]
...
impl SegmentScheduler {
    ...
    /// `file_old_total` (`flist.c:2785-2786`), leaving the difference at 0.
    ...
    /// `flist.c:2375` does, and the loop stops as soon as enough entries are
    /// queued ahead.
    ...
    /// the backlog is under the ceiling (`io.c:854-861`), and when that poll
    /// finds no input ready it calls `send_extra_file_list(sock_f_out, -1)`
    /// (`io.c:873`), whose `at_least` resolves to `backlog + 1` - exactly one
    ...
    /// entire directory per iteration (`flist.c:2667`), and truncating a
    ...
    /// # Upstream Reference
    ///
    /// - `sender.c:532-536` - `file_old_total -= first_flist->used` followed by
...
/// # Upstream Reference
///
/// - `flist.c:2774-2785` - INC_RECURSE sub-list and NDX_FLIST_EOF dispatch
...
pub(crate) struct IncrementalState {
    ...
    /// between them (`flist.c:3209`); this resolves the NDX values the receiver
