...
///
/// Mirrors upstream `flist.c:455-459`: when the daemon module has
...
pub(super) fn strip_symlink_munge_prefix(munge_symlinks: bool, target: PathBuf) -> PathBuf {
    ...
    // upstream: flist.c:499 - the strncmp guard requires `llen > SYMLINK_PREFIX_LEN`,
...
mod munge_strip_tests {
    ...
    fn does_not_strip_exact_prefix_match() {
        // upstream: flist.c:499 - `llen > SYMLINK_PREFIX_LEN` is strict, so a
