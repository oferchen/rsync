//! End-to-end loopback of the INC_RECURSE streaming receiver with non-regular
//! entries: symlinks (including one pointing into another directory's
//! segment) and a FIFO in a nested directory.
//!
//! Same wiring and tree shape as `inc_recurse_receiver_loopback.rs` (a real
//! server sender and client receiver over a socket pair, INC_RECURSE forced
//! by the sender's `-e.i...` flag string). The streaming driver's symlink,
//! and special passes must reproduce every such node.
//!
//! # Upstream Reference
//!
//! - `generator.c:1948-2002` - symlinks created inline in `recv_generator()`.
//! - `generator.c:2031` - devices/specials created inline in `recv_generator()`.
...
/// Adds the non-regular nodes: two symlinks (one dangling, one pointing up a
/// directory) and a FIFO.
fn add_special_nodes(src: &Path) {
    std::os::unix::fs::symlink("f00001", src.join("dir0/ln_sibling")).expect("symlink");
    std::os::unix::fs::symlink("../dir0/f00002", src.join("dir2/ln_up")).expect("symlink");
    std::os::unix::fs::symlink("no/such/target", src.join("dir1/ln_dangling")).expect("symlink");
    let status = std::process::Command::new("mkfifo")
        .arg(src.join("dir1/nested/fifo"))
        .status()
        .expect("run mkfifo");
...
fn list_tree(root: &Path) -> Vec<PathBuf> {
    ...
    while let Some(dir) = stack.pop() {
        for entry in fs::read_dir(&dir).expect("read_dir") {
            ...
            if fs::symlink_metadata(&path).expect("lstat").is_dir() {
                stack.push(path);
            }
        }
    }
...
/// A forced-INC_RECURSE `-rtlD` pull recreates symlinks, a FIFO and a
/// cross-directory symlink through the streaming receiver.
#[test]
fn inc_recurse_receiver_loopback_recreates_links_and_specials() {
    ...
    for rel in &src_entries {
        let s = src.join(rel);
        if fs::symlink_metadata(&s).expect("lstat src").is_file() {
            let d = dst.join(rel);
            assert!(
    ...
    for link in ["dir0/ln_sibling", "dir2/ln_up", "dir1/ln_dangling"] {
        assert_eq!(
            fs::read_link(dst.join(link)).expect("dst symlink"),
            fs::read_link(src.join(link)).expect("src symlink"),
            "symlink {link} target"
        );
    }
    let fifo = fs::symlink_metadata(dst.join("dir1/nested/fifo")).expect("dst fifo");
    assert!(
        fifo.file_type().is_fifo(),
        "dir1/nested/fifo must be a FIFO"
    );
}
