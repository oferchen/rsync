...
/// Emits a branded diagnostic until native server mode is available.
...
fn write_server_unavailable_error<Err: Write>(stderr: &mut Err, brand: Brand) {
    ...
    let mut message = rsync_error!(
        1,
        "native server mode is not yet available; this build does not delegate to an external rsync binary"
    );
    message = message.with_role(Role::Server);

    if super::write_message(&message, &mut sink).is_err() {
        let _ = writeln!(sink.writer_mut(), "{}", message.text());
    }
}
