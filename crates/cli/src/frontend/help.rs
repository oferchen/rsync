...
pub(super) fn help_text(program_name: ProgramName) -> String {
    ...
    format!(
        concat!(
            ...
            "      --aes        Force AES ciphers for SSH even without hardware AES detection.\n",
            "      --no-aes     Disable automatic AES cipher selection for SSH.\n",
            "      --ssh-cipher=CIPHERS  Comma-separated cipher preference list for embedded SSH.\n",
            "      --ssh-connect-timeout=SECS  Connection timeout in seconds for embedded SSH.\n",
            "      --ssh-keepalive=SECS  Keepalive interval in seconds for embedded SSH (0 = disable).\n",
            "      --ssh-identity=FILE  Identity file for embedded SSH (repeatable).\n",
            "      --ssh-no-agent  Disable SSH agent authentication for embedded SSH.\n",
            "      --ssh-strict-host-key-checking=MODE  Host key verification policy for embedded SSH (yes, no, ask).\n",
            "      --ssh-ipv6   Prefer IPv6 for embedded SSH connections.\n",
            "      --ssh-port=PORT  Port override for embedded SSH connections.\n",
            "      --jump-host=[user@]HOST[:PORT][,...]  Comma-separated ProxyJump hosts for embedded SSH.\n",
            "\n",
            "    Performance & I/O:\n",
