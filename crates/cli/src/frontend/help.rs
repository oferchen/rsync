...
pub(super) fn help_text(program_name: ProgramName) -> String {
    ...
    let mut help = format!(
        concat!(
            ...
            "      --ssh-strict-host-key-checking=MODE  Host key verification policy (yes, no, ask, accept-new; true/false/off also accepted) for oc's embedded ssh:// transport (ssh:// URL operands only); standard host:path SSH uses ssh_config StrictHostKeyChecking or the -e/--rsh remote shell.\n",
            ...
            "      --parallel-delta-scan  Opt-in: scan a large file's delta across multiple cores (sender side). Only engages for large, duplicate-free basis files (duplicate-content basis files fall back to the sequential scan, as does --inplace, whose in-place basis updates require monotonic basis reads). Reconstruction and matched/literal stats are unaffected; the literal-token wire framing may differ by a few bytes at a range boundary. Local-only, never forwarded to a remote peer. Default off.\n",
