...
pub(super) fn help_text(program_name: ProgramName) -> String {
    ...
    format!(
        concat!(
            ...
            "\n",
            "    Connection / SSH:\n",
            "      --connect-program=COMMAND  CLI equivalent of the RSYNC_CONNECT_PROG environment variable, which oc-rsync also honors (%H = host, %P = port). Not a remote shell - use -e/--rsh for shell transfers.\n",
            "      --tcp-fastopen=MODE  Enable TCP Fast Open on daemon and client sockets\n",
            "                              (auto, on, off; default auto: enabled where supported).\n",
            "      --aes        Force AES-GCM ciphers for embedded ssh:// transfers even without hardware AES detection; for host:path targets select ciphers via -e 'ssh -c ...'.\n",
            "      --no-aes     Disable automatic AES-GCM cipher selection for embedded ssh:// transfers; for host:path targets select ciphers via -e 'ssh -c ...'.\n",
            "      --ssh-cipher=CIPHERS  Comma-separated cipher preference list for embedded ssh:// transfers only; for host:path targets use -e 'ssh -c ...' or ssh_config Ciphers.\n",
            "      --ssh-connect-timeout=SECS  Connection timeout in seconds for embedded ssh:// transfers; distinct from --contimeout (rsync-level) and from -e 'ssh -o ConnectTimeout='.\n",
            "      --ssh-keepalive=SECS  Keepalive interval in seconds for embedded ssh:// transfers only (0 = disable); for host:path targets use -e 'ssh -o ServerAliveInterval=N'.\n",
            "      --ssh-identity=FILE  Identity file for embedded ssh:// transfers only (repeatable); for host:path targets use -e 'ssh -i FILE' or ssh_config IdentityFile.\n",
            "      --ssh-no-agent  Disable SSH agent authentication for embedded ssh:// transfers only; for host:path targets use -e 'ssh -o IdentityAgent=none'.\n",
            "      --ssh-strict-host-key-checking=MODE  Host key verification policy (yes, no, ask) for embedded ssh:// transfers only; for host:path targets use -e 'ssh -o StrictHostKeyChecking=...' or ssh_config.\n",
            "      --ssh-ipv6   Prefer IPv6 for embedded ssh:// transfers only; rsync's own -6/--ipv6 covers daemon and other transports.\n",
            "      --ssh-port=PORT  Port override for embedded ssh:// transfers only; for host:path targets set the port via -e 'ssh -p PORT' or ssh_config.\n",
            "      --jump-host=[user@]HOST[:PORT][,...]  Comma-separated proxy-jump hosts for embedded ssh:// transfers (forwarded as ssh -J); for host:path targets pass -e 'ssh -J ...' or set ssh_config ProxyJump.\n",
            "\n",
            "    Performance & I/O:\n",
