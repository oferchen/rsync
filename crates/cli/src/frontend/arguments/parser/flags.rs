...
///
/// upstream: options.c:1659 `case 'a'` expands `-a` in place during the argv
...
/// and `--no-foo` resets `level = 0` (e.g. options.c:1591 `++preserve_atimes`,
/// options.c:1883 `preserve_xattrs++`). The level is capped at 2 because that is
