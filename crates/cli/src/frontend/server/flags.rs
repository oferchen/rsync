...
///
/// upstream: options.c:1497 - the server runs the same popt parser as the
...
///
/// Upstream rsync's `server_options()` (options.c:2613) embeds `s` in the
...
pub(crate) fn detect_secluded_args_flag(args: &[OsString]) -> bool {
    ...
    options.iter().skip(1).any(|a| {
        ...
        // not secluded-args.
        // upstream: options.c:2613 - protect_args placed at argstr[1]
...
pub(super) struct ServerLongFlags {
    ...
    ///
    /// upstream: options.c:2976 - `server_options()` emits `--bwlimit=%d` where
    /// `%d` is the rate in whole KiB (options.c:1724). Only the sender-role
    /// server (`--sender`) acts on it; a receiver disables its own throttle
    /// (main.c:1081). Captured raw here and parsed when building the config.
    ...
    ///
    /// upstream: options.c:2903 - `server_options()` emits bare `--partial`
    ...
    ///
    /// upstream: options.c:2770-2775 - the compact `D` letter carries
    ...
    ///
    /// upstream: options.c:2855-2856 - `--max-alloc=arg` is emitted by
    ...
    /// popt entry), :2382-2399 (validation), syscall.c:163
    ...
    /// server_options() (options.c:2961-2964) always emits the bare `--append`
    /// long flag (never `--append-verify`); the receiver seeks past the
    /// existing length and the sender streams only the tail (sender.c:90-99,
    /// generator.c:786).
    ...
    ///
    /// upstream: options.c:1730 - a second `--append` on the server bumps
    /// `append_mode` to 2, folding the on-disk prefix into the whole-file
    /// checksum (match.c:373, receiver.c:370) so a corrupted prefix fails
    ...
    ///
    /// upstream: options.c:2974-2975 - `server_options()` emits
    ...
    /// server_options() forwards the bare long flag (options.c:3083-3084).
    ...
    ///
    /// upstream: options.c:2848-2849 - `server_options()` emits `--stats` whenever
    ...
    ///
    /// upstream: options.c:2726-2733 - client forwards the post-comma half of
    ...
    /// upstream: options.c:2925-2933 - `--compare-dest`, `--copy-dest`, `--link-dest`
    pub(super) reference_directories: Vec<ReferenceDirectory>,
    /// Raw `--info=FLAGS` values forwarded by the client.
    ///
    /// upstream: options.c:2938-2941 - `server_options()` emits the output of
    ...
    /// (options.c:1783 `parse_output_words(debug_words, ...)`), silently
    ...
    ///
    /// upstream: options.c:2810-2815 - `server_options()` emits long-form
    ...
    ///
    /// upstream: options.c:2764-2768 - `server_options()` emits
    ...
    ///
    /// upstream: options.c:2778-2790 - the client sends `--log-format=%i`
    ...
    ///
    /// upstream: options.c:2896-2900 - `server_options()` emits the option
    ...
    ///
    /// upstream: options.c:2901-2902 - `server_options()` emits
    ...
    ///
    /// upstream: options.c:3006-3007 - `if (mkpath_dest_arg && am_sender)
    /// args[ac++] = "--mkpath"`. Long-form only. Gates the receiver's
    /// dest-arg path creation (`main.c:749` `make_path` vs `main.c:801`
    /// single `do_mkdir`).
    pub(super) mkpath: bool,
    /// Whether the client forwarded `--list-only` (upstream `list_only > 1`).
    ///
    /// upstream: options.c:2757-2758 - forwarded so the peer knows the transfer
    ...
    ///
    /// upstream: options.c:2860-2861 - `if (write_batch < 0) args[ac++] =
    /// "--only-write-batch=X"`. On the receiver, main.c:1866 forces
    ...
    /// batch locally (sender.c:220) and streams no delta data over the wire.
    pub(super) only_write_batch: bool,

    /// Whether the client forwarded `--no-implied-dirs` (upstream
    /// `implied_dirs == 0`).
    ///
    /// upstream: options.c:2986-2987 - forwarded to the sender on a pull. As the
    /// server-side sender, this process must omit the implied parent dirs from
    /// the flist at protocol < 30 (flist.c:2708); protocol >= 30 always sends
    /// them (flist.c:2496-2497).
    pub(super) no_implied_dirs: bool,

    /// Whether the client forwarded `--no-r` (upstream `recurse = 0`).
    ///
    /// upstream: options.c:2760-2763 - `if (xfer_dirs && !recurse &&
    ...
    ///
    /// upstream: options.c:2965-2969 - under `--inplace`, `if (sparse_files &&
    ...
    /// `missing_args == 2`).
    ///
    /// upstream: options.c:2878-2879 - `if (missing_args == 2) args[ac++] =
    ...
    /// `missing_args == 1`).
    ///
    /// upstream: options.c:2880-2881 - `else if (missing_args == 1 && !am_sender)
    ...
    ///
    /// upstream: options.c:2822-2823 - `server_options()` emits
    ...
    /// # Upstream Reference
    ///
    /// - `rsync-3.5.1/options.c:2817-2818` - `safe_arg("--backup-dir", backup_dir)`.
    pub(super) backup_dir: Option<String>,
    /// The `--temp-dir` value the client forwarded.
    ///
    /// # Upstream Reference
    ///
    /// - `rsync-3.5.1/options.c:2936-2937` - `safe_arg("--temp-dir", tmpdir)`.
    pub(super) temp_dir: Option<String>,

    /// User id/name map spec forwarded by the client (upstream: `--usermap=SPEC`).
    ///
    /// upstream: options.c:3094-3095 - `safe_arg("--usermap", usermap)` (joined
    ...
    ///
    /// upstream: options.c:3097-3098 - `safe_arg("--groupmap", groupmap)` (joined
    ...
    /// `--skip-compress=LIST`).
    ///
    /// upstream: options.c:3036-3037 - `safe_arg("--skip-compress", skip_compress)`
    ...
    ///
    /// upstream: options.c:2964-2967 - `asprintf(&arg, "-B%u", block_size)`
...
pub(super) fn parse_server_long_flags(args: &[OsString]) -> ServerLongFlags {
    ...
    while idx < args.len() {
        ...
        match s.as_ref() {
            ...
            // upstream: options.c:2903 - bare --partial (no compact 'P').
            "--partial" => flags.partial = true,
            // upstream: options.c:2770-2775 - --specials / --no-specials convey
            ...
            // upstream: options.c:2880-2881 - `--ignore-missing-args`
            ...
            // upstream: options.c:2862-2863 - `if (am_root > 1) args[ac++] =
            ...
            // --files-from entry to its basename (flist.c:2578-2589) with no
            // implied parent directories (options.c:2216-2217).
            ...
            // upstream: options.c:1728-1732 - OPT_APPEND increments append_mode
            // on the server side. server_options() (options.c:2961-2964) emits a
            ...
            // upstream: --numeric-ids is long-form only (options.c:2897-2898)
            "--numeric-ids" => flags.numeric_ids = true,
            // upstream: --delete variants are long-form only (options.c:2828-2837)
            "--delete" | "--delete-during" => {
                flags.delete = true;
            }
            // upstream: options.c:3012-3013 server_options() forwards an explicit
            // --delete-before verbatim; compat.c:174-176 keys on it.
            "--delete-before" => {
                flags.delete = true;
                flags.delete_before = true;
            }
            // upstream: options.c:3020-3023 server_options() emits `--delete`
            ...
            // options.c:2343, where `delete_mode || delete_excluded` turns
            ...
            // upstream: options.c:2974-2975 - --remove-source-files is long-form
            ...
            // upstream: options.c:2997 - `--copy-devices` is forwarded to the
            ...
            // upstream: options.c:2848-2849 - --stats forwarded by server_options()
            // when do_stats was set. The server-side flag drives NDX_DEL_STATS
            // emission in the goodbye phase (generator.c:2377,2422).
            "--stats" => flags.stats = true,
            // upstream: options.c:2841 - --ignore-existing sent as long-form arg
            "--ignore-existing" => flags.ignore_existing = true,
            // upstream: options.c:2843 - --existing (--ignore-non-existing) sent as long-form arg
            "--existing" | "--ignore-non-existing" => flags.existing_only = true,
            // upstream: options.c:2810-2815 - non-ZLIB compression algorithms
            ...
            // upstream: options.c:2896-2900 - server_options() emits
            ...
            // upstream: options.c:2901-2902 - emitted alongside --partial-dir.
            "--delay-updates" => flags.delay_updates = true,
            // upstream: options.c:3006-3007 - --mkpath forwarded to the server
            ...
            // upstream: options.c:2757-2758 - `--list-only` forwarded when the
            ...
            // filter. server_options() (options.c:2614) never forwards these
            ...
            _ => {
                ...
                // upstream: options.c:2963-2964 - block_size arrives as a
...
/// # Upstream Reference
///
/// - `options.c:2817-2818` - `--backup-dir`
/// - `options.c:2936-2937` - `--temp-dir`
/// - `options.c:2949-2950` - `--copy-dest` / `--link-dest` / `--compare-dest`
///   via `alt_dest_opt(0)` + `safe_arg("", basis_dir[i])`
/// - `options.c:2974-2975` - `--files-from`
...
fn parse_value_bearing_flag(s: &str, flags: &mut ServerLongFlags) {
    if let Some(value) = s.strip_prefix("--checksum-seed=") {
        ...
        // upstream: options.c:2976 - client forwards `--bwlimit=%d` in whole KiB.
        ...
        // upstream: options.c:2817-2818 - safe_arg("--backup-dir", backup_dir).
    ...
    // upstream: options.c:2810-2815 - `--compress-choice=ALGO` / `--zc=ALGO`
    ...
    // upstream: options.c:2932-2936 - `--compress-level=%d` carries the
    ...
    // upstream: options.c:2778-2790 - client forwards --log-format=%i (or %o,
    // %i%I, X) so the server knows whether to generate itemize data.
    } else if s.strip_prefix("--only-write-batch=").is_some() {
        // upstream: options.c:2860-2861 - server_options() always emits the
    ...
    // upstream: options.c:2938-2941 - server_options() forwards info levels.
    ...
    // upstream: options.c:2925-2933 - reference directory args
...
pub(super) fn is_known_server_long_flag(arg: &str) -> bool {
    matches!(
        ...
        "--server"
            ...
            | "--no-msgs2stderr"
            | "--qsort"
            // upstream: options.c:2918-2919 - the spelling server_options()
            // actually emits for use_qsort (oc's own forwarder uses --qsort).
            | "--use-qsort"
            // upstream: options.c:3003-3004 - `--open-noatime` (sender O_NOATIME).
            | "--open-noatime"
            // upstream: options.c:2858-2859 - `--force` (force_delete), am_sender
            // block so it reaches a server receiver.
            | "--force"
            // upstream: options.c:2862-2863 - `--super` (am_root > 1).
            | "--super"
            ...
            | "--fake-super"
            | "--no-fake-super"
            // upstream: options.c:3000-3001 - `--preallocate` (preallocate_files).
            | "--preallocate"
            // upstream: options.c:2878-2881 - missing-args cooperation flags.
            | "--delete-missing-args"
            | "--ignore-missing-args"
            | "--no-implied-dirs"
            // upstream: options.c:2763/2969/2983 - server_options() emits these
        ...
        // upstream: options.c:2817-2818 / 2926-2927 - server_options() emits
        ...
        // upstream: options.c:2884 - a negative modify_window arrives as the
        ...
        // upstream: options.c:2976-2977 - server_options() emits `--bwlimit=%d`
        ...
        // upstream: options.c:2822-2823 / 2912-2913 / 2915-2916 / 2859-2860 -
        ...
        // upstream: options.c:2963-2964 - block_size arrives as a standalone
        ...
        // sink (upstream calls `log_init(0)` from options.c:2539), so storing
        ...
        // upstream: options.c:1783 - `--debug=FLAGS` parsed via
        ...
        // upstream: options.c:2860-2861 - `--only-write-batch=X` reaches a
...
///
/// Only `-e` qualifies. `maybe_add_e_option()` (options.c:3031) appends the
...
/// suffix (`e.<caps>`, options.c:2738) rides inside the token and the string
...
/// - `options.c:2817-2818` - `--backup-dir`
/// - `options.c:2896-2900` - `--partial-dir`
/// - `options.c:2936-2937` - `--temp-dir`
/// - `options.c:2949-2950` - `--copy-dest` / `--link-dest` / `--compare-dest`
///   (via `alt_dest_opt(0)`)
/// - `options.c:2974-2975` - `--files-from`
