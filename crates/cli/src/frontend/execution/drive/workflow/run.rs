...
use core::{
    client::{FEATURE_UNAVAILABLE_EXIT_CODE, HumanReadableMode},
    message::Role,
    rsync_error,
};
    ...
    Err: Write,
{
    ...
    let requires_remote = transfer_requires_remote(&remainder, &file_list_operands);
    if requires_remote || batch_mode_requested {
        let message = rsync_error!(
            FEATURE_UNAVAILABLE_EXIT_CODE,
            "remote or batch transfers are not yet supported by oc-rsync"
        )
