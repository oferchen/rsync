    ...
    let command = command
        ...
        .arg(
            Arg::new("connect-program")
                ...
                .value_name("COMMAND")
                .help(
                    "CLI equivalent of the RSYNC_CONNECT_PROG environment variable, which oc-rsync also honors (%H = host, %P = port). Not a remote shell - use -e/--rsh for shell transfers.",
                )
                .num_args(1)
                .action(ArgAction::Set)
