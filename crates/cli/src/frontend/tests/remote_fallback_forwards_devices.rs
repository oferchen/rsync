...
"#;
    ...
    assert!(args.contains(&"--copy-devices"));
    ...
    let recorded = std::fs::read_to_string(&args_path).expect("read args file");
    assert!(!recorded.lines().any(|line| line == "--copy-devices"));
}

#[cfg(unix)]
#[test]
fn remote_fallback_forwards_write_devices_flag() {
    use tempfile::tempdir;
...
"#;
    ...
    let recorded = std::fs::read_to_string(&args_path).expect("read args file");
    let args: Vec<&str> = recorded.lines().collect();
    assert!(args.contains(&"--write-devices"));
}
