...
#[cfg(unix)]
#[test]
fn remote_fallback_forwards_compress_choice() {
    use tempfile::tempdir;
...
"#;
    ...
    let (code, stdout, stderr) = run_with_args([
        OsString::from(RSYNC),
        OsString::from("-z"),
        OsString::from("--compress-choice=zlib"),
        OsString::from("remote::module"),
        OsString::from("dest"),
    ]);
    ...
    assert!(seen_choice, "--compress-choice argument not forwarded");
}

#[cfg(unix)]
#[test]
fn remote_fallback_forwards_open_noatime_flag() {
    use tempfile::tempdir;
...
#[cfg(unix)]
#[test]
fn remote_fallback_force_no_compress_overrides_cli() {
    use std::ffi::OsStr;
    use tempfile::tempdir;
...
"#;
    ...
    let _force_guard = EnvGuard::set("OC_RSYNC_FORCE_NO_COMPRESS", OsStr::new("1"));
    let _fallback_guard = EnvGuard::set(CLIENT_FALLBACK_ENV, script_path.as_os_str());
    let _args_guard = EnvGuard::set("ARGS_FILE", args_path.as_os_str());
    ...
    assert!(!lines.contains(&"--compress-choice=zlib"));
}
