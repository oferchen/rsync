...
//! `--version`) and reports the workspace's limited runtime capabilities when a
//! transfer is requested. Higher layers will eventually extend the parser to
//! cover the full upstream surface, but providing these entry points today
//! allows downstream tooling to depend on a stable binary path (`rsync`) while
...
//! mirroring the approach used by upstream rsync. Internally a [`clap`](https://docs.rs/clap/)
//! command definition performs a light-weight parse that recognises `--help`
//! and `--version` flags while treating all other tokens as transfer arguments.
//! When a transfer is requested, the function delegates to
//! [`rsync_core::client::run_client`], which emits a structured
//! [`Message`](rsync_core::message::Message) explaining that the delta-transfer
//! engine has not yet been implemented.
//!
//! # Invariants
//!
...
//!   diagnostics remain centralised while the engine is under construction.
//!
//! # Errors
//!
//! The parser returns a diagnostic message with exit code `1` when argument
//! processing fails. Transfer attempts produce exit code `1` alongside a
//! descriptive error routed through [`rsync_core::message`].
//!
//! # Examples
//!
//! ```
...
mod tests {
    ...
    fn transfer_request_reports_missing_engine() {
        ...
        assert!(rendered.contains("rsync error: client functionality is unavailable"));
