...
fn extract_extension_no_extension() {
    assert!(CompressionDecider::extract_extension(Path::new("Makefile")).is_none());
}
...
fn suffix_normalization() {
    ...
    let s = Suffix::new("  .Tar.GZ  ");
    assert_eq!(s.as_str(), "tar.gz");

    let s = Suffix::new("txt");
    assert_eq!(s.as_str(), "txt");
}

#[test]
fn suffix_equality() {
    assert_eq!(Suffix::new("jpg"), Suffix::new(".JPG"));
    assert_eq!(Suffix::new("tar.gz"), Suffix::new(".TAR.GZ"));
    assert_ne!(Suffix::new("jpg"), Suffix::new("png"));
}

#[test]
fn suffix_display() {
    let s = Suffix::new(".PDF");
    assert_eq!(format!("{s}"), "pdf");
}
