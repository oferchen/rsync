...
impl<R: Read> NegotiatedStream<R> {
    ...
    ///
    /// The method mirrors [`protocol::read_legacy_daemon_line`] but operates on the
    /// replaying stream wrapper instead of a [`protocol::NegotiationPrologueSniffer`]. It expects the
    ...
    /// [`protocol::parse_legacy_daemon_greeting_bytes`]. On success the negotiated
    ...
    ///
    /// The helper mirrors [`protocol::parse_legacy_daemon_message_bytes`] but operates on the
    ...
    /// [`protocol::parse_legacy_error_message_bytes`]. Any parsing failure is converted into
