...
fn xattrs_supported(path: &Path) -> bool {
    ...
    if apply_xattrs_from_list(path, &list, false, None, None, None).is_err() {
        return false;
    }
    ...
    let _ = apply_xattrs_from_list(path, &cleared, false, None, None, None);
    true
}
...
fn simulated_windows_xattr_applied_on_windows() {
    ...
    apply_xattrs_from_list(&file, &list, false, None, None, None).expect("apply xattrs on Windows");
