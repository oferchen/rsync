...
/// upstream: options.c:2401 - daemon_bwlimit override: strictest rate wins
pub fn apply_effective_limit(
    ...
    if let Some(limit) = limit {
        if let Some(existing) = limiter {
            ...
            if target_limit != existing.limit_bytes() {
                existing.update_limit(target_limit);
                return LimiterChange::Updated;
            }
            LimiterChange::Unchanged
        } else {
            *limiter = Some(BandwidthLimiter::new(limit));
            LimiterChange::Enabled
        }
    } else if limiter.take().is_some() {
...
mod tests {
    ...
    fn apply_effective_limit_unchanged_when_nothing_specified() {
        ...
        let result = apply_effective_limit(&mut limiter, None, false);
    ...
    fn apply_effective_limit_enables_limiter_when_none_exists() {
        let mut limiter = None;
        let result = apply_effective_limit(&mut limiter, Some(nz(1000)), true);
    ...
    fn apply_effective_limit_disables_limiter_when_limit_is_none() {
        let mut limiter = Some(BandwidthLimiter::new(nz(1000)));
        let result = apply_effective_limit(&mut limiter, None, true);
    ...
    fn apply_effective_limit_unchanged_when_limit_higher() {
        ...
        let mut limiter = Some(BandwidthLimiter::new(nz(1000)));
        let result = apply_effective_limit(&mut limiter, Some(nz(2000)), true);
        assert_eq!(result, LimiterChange::Unchanged);
        assert_eq!(limiter.as_ref().unwrap().limit_bytes().get(), 1000);
    }
    ...
    fn apply_effective_limit_very_small_limit() {
        let mut limiter = None;
        let result = apply_effective_limit(&mut limiter, Some(nz(1)), true);
        assert_eq!(result, LimiterChange::Enabled);
    ...
    fn apply_effective_limit_very_large_limit() {
        let mut limiter = None;
        let result = apply_effective_limit(&mut limiter, Some(nz(u64::MAX)), true);
        assert_eq!(result, LimiterChange::Enabled);
    ...
    fn apply_effective_limit_repeated_updates() {
        ...
        // First enable
        let r1 = apply_effective_limit(&mut limiter, Some(nz(2000)), true);
        assert_eq!(r1, LimiterChange::Enabled);

        // Update to lower
        let r2 = apply_effective_limit(&mut limiter, Some(nz(1500)), true);
        assert_eq!(r2, LimiterChange::Updated);

        // Try to update to higher (should be unchanged due to min)
        let r3 = apply_effective_limit(&mut limiter, Some(nz(2000)), true);
        assert_eq!(r3, LimiterChange::Unchanged);

        // Disable
        let r4 = apply_effective_limit(&mut limiter, None, true);
        assert_eq!(r4, LimiterChange::Disabled);

        // Re-enable
        let r5 = apply_effective_limit(&mut limiter, Some(nz(1000)), true);
