#[test]
fn run_daemon_rejects_push_to_default_read_only_module() {
    ...
    // Omit `read only` directive - upstream defaults to true.
    ...
    // Request the module that has no explicit read-only setting
    stream
        .write_all(b"defaultro\n")
    ...
    // for protocol >= 30.
    stream
        .write_all(b"--server\0-logDtpr\0.\0defaultro/\0\0")
    ...
    // `do_server_recv()` (main.c:1201-1205).
