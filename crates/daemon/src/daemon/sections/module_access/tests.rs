...
mod module_access_tests {
    ...
    /// for that terminator (`exclude.c:1225-1226`), one byte further along
    /// than the `- 1` it returns for a space, so the scan at
    /// `exclude.c:1364-1443` reads what follows as MODIFIER characters.
    ///
    ...
    ///
    /// upstream: `parse_rule_tok`'s `default:` arm (`exclude.c:1322-1326`)
    /// takes ANY single character as `ch`, and the second switch
    /// (`exclude.c:1345-1357`) gives `H`/`S` FILTRULE_SENDER_SIDE and
    ...
    /// after it), so the scan's first `*++s` (`exclude.c:1364`) reads the very
    ...
    fn a_modifier_run_after_a_short_prefix_is_accepted() {
        ...
        // `/` presets FILTRULE_ABS_PATH (exclude.c:1391-1392), anchoring the
    ...
    ///
    /// upstream: `prefix_specifies_side` gates `r`, `s` (`exclude.c:1423-1430`)
    ...
    ///
    /// upstream: the `default:` arm of `parse_rule_tok` (`exclude.c:1322-1326`)
