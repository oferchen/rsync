...
// receiver. These tests lock the runtime-handle acquisition contract that lets
// `run_daemon_transfer` route through `run_server_with_handshake_on` while
// staying byte-identical: the shim must yield a *usable* handle without building
// a nested runtime inside an ambient one. Gated on `tokio-transfer` so the
// default build compiles neither the shim nor its tests.
#[cfg(all(test, feature = "tokio-transfer"))]
mod tokio_reroute_tests {
    use super::*;

    /// With no ambient runtime (the threaded `serve_connections` worker path),
    /// the shim builds a session-scoped current-thread runtime and drives the
    /// closure's own `block_on` on the supplied handle - the exact shape the
    /// daemon receiver uses to reach the tokio driver. A failure here would mean
    /// the socket-backed receiver cannot obtain a runtime and would panic
    /// instead of routing byte-identically.
    #[test]
    fn shim_builds_runtime_when_none_ambient() {
        let out = with_daemon_transfer_runtime(|handle| handle.block_on(async { 11u32 }));
        assert_eq!(out, 11);
    }

    /// Inside a `spawn_blocking` worker of the hybrid async listener,
    /// `Handle::current()` resolves, so the shim must *adopt* that ambient
    /// runtime rather than build a second one. Adopting is required for
    /// byte-identity: a nested `block_on` on the current-thread flavour would
    /// panic, and building a rival runtime would break the single-runtime
    /// ownership rule. `block_in_place` proves the adopted handle is the ambient
    /// one without a nested `block_on`.
    #[test]
    fn shim_adopts_ambient_runtime() {
        ...
        let adopted_id = rt.block_on(async {
            tokio::task::block_in_place(|| {
                with_daemon_transfer_runtime(|handle| handle.id())
            })
        });
        assert_eq!(
