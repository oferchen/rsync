...
fn parse_daemon_filter_token(token: &str) -> Result<Option<FilterRuleWireFormat>, MalformedRule> {
    ...
    // `+ - P R S H . : !` character a rule character (exclude.c:1322-1326):
    //
    //     default:
    //             ch = *s;
    //             if (s[1] == ',')
    ...
    if let Some((prefix, ch)) = short_rule_prefix(token) {
        ...
        match scan_modifiers(run, prefix.specifies_side, base, token) {
            Ok((modifiers, used)) => {
                return build_prefixed_rule(
                    prefix,
                    modifiers,
                    run,
                    used,
                    token,
                    // A bare `-`/`+` already refused before this change, and
                    // that is upstream's answer; keep it.
                    EmptyPattern::Refuse,
                );
            }
            ...
            // TERMINATOR (`rule_strcmp`, exclude.c:1225), so `P,` is a rule
            ...
            // prefix, exclude.c:1423-1430) and, with the fall-through taken
    ...
    // upstream: exclude.c:1289-1324 - keyword-to-short-form mapping;
    // `hide`/`show` set FILTRULE_SENDER_SIDE (exclude.c:1345-1350),
    // `protect`/`risk` FILTRULE_RECEIVER_SIDE (exclude.c:1351-1357). All four
    // set `prefix_specifies_side`, which narrows the modifier alphabet.
    for &(keyword, prefix) in KEYWORD_PREFIXES {
        if let Some(rest) = strip_matched_keyword(token, keyword) {
            ...
            return build_prefixed_rule(
                ...
                used,
                token,
                // Unchanged from before: a patternless keyword is skipped, not
                // refused. Upstream refuses it; closing that is its own change.
                EmptyPattern::Skip,
            );
        }
    }

    // `clear` maps to `!` (exclude.c:1290-1291), and `!` SKIPS the modifier
    // scan entirely (`while (ch != '!' && ...)`, exclude.c:1364) - so no
    // modifier handling belongs on this arm. What upstream does with a
    // TRAILING remainder here is a separate question this change does not
    // touch.
    if strip_matched_keyword(token, "clear").is_some() {
...
/// belongs to the gluing splitter, not to this function.
...
/// (`exclude.c:1328-1362`). The long keywords are pure aliases for the short
/// characters (`exclude.c:1289-1319`), so ONE table describes both.
#[derive(Clone, Copy)]
struct RulePrefix {
    ...
    /// `s` invalid modifiers (`exclude.c:1403-1404`, `:1423-1430`).
    specifies_side: bool,
...
///
/// upstream: `exclude.c:1322-1326` makes any single character a rule
/// character, and the second switch (`exclude.c:1328-1362`) gives
...
fn opens_short_rule(s: &str) -> bool {
    ...
    // A `,` terminates the prefix outright (`rule_strcmp`, exclude.c:1225), so
...
struct RuleModifiers {
    /// `/` - `FILTRULE_ABS_PATH` (`exclude.c:1391-1392`).
    abs_path: bool,
    /// `s` - `FILTRULE_SENDER_SIDE` (`exclude.c:1431-1435`).
    sender_side: bool,
...
///   short-circuits on `FILTRULE_CVS_IGNORE`, `exclude.c:1261-1266`).
...
fn scan_modifiers(
    ...
    for (offset, ch) in run.char_indices() {
        ...
        // any `isspace` byte too (exclude.c:1365-1368).
...
/// What an empty pattern means on a given path.
///
/// upstream refuses BOTH (`else if (!len && !CVS_IGNORE)`,
/// `exclude.c:1474-1476`). oc's KEYWORD path still skips such a rule silently;
/// closing that fail-open is a separate change, so this enum keeps the two
/// paths' existing answers apart instead of quietly changing one of them here.
