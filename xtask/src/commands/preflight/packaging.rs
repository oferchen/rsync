...
pub(super) fn validate_packaging(workspace: &Path, branding: &WorkspaceBranding) -> TaskResult<()> {
...
fn validate_packaging_assets(workspace: &Path, branding: &WorkspaceBranding) -> TaskResult<()> {
    ...
    validate_environment_defaults(workspace)?;
...
fn validate_environment_defaults(workspace: &Path) -> TaskResult<()> {
