...
//!
//! Invocations follow the conventional `cargo xtask <command>` pattern. Each
//! subcommand lives in the [`crate::commands`] module to keep the binary modular
//! and testable. The `sbom` command executes the installed `cargo-cyclonedx`
//! plugin with the workspace manifest and writes the resulting JSON document to
    ...
    I: IntoIterator<Item = OsString>,
{
    ...
    match command.as_ref() {
        ...
        "branding" => {
            let options = branding::parse_args(remaining.iter().cloned())?;
            let workspace = workspace_root()?;
            branding::execute(&workspace, options)
        }
        "docs" => {
            let options = docs::parse_args(remaining.iter().cloned())?;
            let workspace = workspace_root()?;
            docs::execute(&workspace, options)
        }
        "no-binaries" => {
            let options = no_binaries::parse_args(remaining.iter().cloned())?;
            let workspace = workspace_root()?;
            no_binaries::execute(&workspace, options)
        }
        "enforce-limits" => {
            let options = enforce_limits::parse_args(remaining.iter().cloned())?;
            let workspace = workspace_root()?;
            enforce_limits::execute(&workspace, options)
        }
        "no-placeholders" => {
            let options = no_placeholders::parse_args(remaining.iter().cloned())?;
            let workspace = workspace_root()?;
            no_placeholders::execute(&workspace, options)
        }
        "preflight" => {
            let options = preflight::parse_args(remaining.iter().cloned())?;
            let workspace = workspace_root()?;
            preflight::execute(&workspace, options)
        }
        "sbom" => {
            let options = sbom::parse_args(remaining.iter().cloned())?;
            let workspace = workspace_root()?;
            sbom::execute(&workspace, options)
        }
        "package" => {
            let options = package::parse_args(remaining.iter().cloned())?;
            let workspace = workspace_root()?;
...
fn top_level_usage() -> String {
    String::from(
        ...
        \n\
        Available commands:\n\
          branding         Inspect workspace branding metadata\n\
          docs             Build API documentation and run doctests\n\
          enforce-limits   Enforce Rust source line-count limits\n\
          no-binaries      Fail if tracked binaries are present\n\
          no-placeholders  Ensure placeholder markers are absent from Rust sources\n\
          preflight        Run the workspace preflight checks\n\
          sbom             Generate a CycloneDX software bill of materials\n\
          package          Build release-ready packages\n\
        \n\
...
mod tests {
    ...
    #[test]
    fn run_with_args_reports_missing_command() {
        let error = run_with_args(std::iter::empty::<OsString>()).unwrap_err();
