...
- **URV-5.b.1 alt-basis containment** - The lexical pre-check in `validate_client_paths_in_module()` also covers the alt-basis source flags `--compare-dest` / `--copy-dest` / `--link-dest` (plus their `--*-destination` long-form spellings, which is how upstream serialises them on the wire). Combined with the `DirSandbox` open-time enforcement (URV-5.a), this gives a two-layer defense against module containment escapes: the wire-protocol layer rejects an obviously out-of-tree path before any I/O is attempted, and the open-time sandbox catches any path that slipped through (e.g. a symlink swap inside the module root that resolves outside it). A client that passes `--copy-dest=/etc` to a non-chroot daemon receives `@ERROR: --copy-dest path '/etc' is outside module root` and the connection is closed without opening any file.
