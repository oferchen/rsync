...
- **Daemon TLS.** Native TLS is available behind the `daemon-tls` and `client-tls` feature flags (rustls-based, no OpenSSL dependency). When enabled, the daemon accepts TLS connections directly via `ssl cert` / `ssl key` / `ssl ca` directives in `oc-rsyncd.conf`, and the client connects with `--ssl`. Default builds do not include TLS. External wrapping with `stunnel`, `ssh -L`, or a reverse proxy remains fully supported for deployments that prefer it. See [`docs/user/daemon-tls-wrapping.md`](./docs/user/daemon-tls-wrapping.md) for setup instructions covering both native and external TLS.
...
| `daemon-tls` | workspace, `daemon` | no | Native TLS acceptor for daemon mode via rustls. Adds `ssl cert`, `ssl key`, `ssl ca` directives to `oc-rsyncd.conf`. | stable |
...
| `parallel-receive-delta` | `engine`, `transfer` | yes | Rayon-dispatched per-file receive-side delta application via `ParallelDeltaApplier`. Fans out per-chunk strong-checksum verification across rayon workers while serializing writes through a per-file reorder buffer - verification scales with core count, writes stay deterministic. Wired into the production receiver `token_loop` via PIP-9; validated through the full upstream interop matrix (PIP-10.a), adversarial stress tests (PIP-10.b), mixed-size correctness (PIP-10.c), RSS overhead measurement (PIP-10.d), and error-path parity (PIP-10.e). The feature flag is retained temporarily as an emergency opt-out; build without the feature to fall back to the sequential path. See [`docs/design/parallel-receive-delta-bake-prerequisites.md`](./docs/design/parallel-receive-delta-bake-prerequisites.md) for the bake-window evidence. | stable (default-on) |
