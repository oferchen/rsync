...
| `parallel-receive-delta` | `engine`, `transfer` | no | Rayon-dispatched per-file receive-side delta application via `ParallelDeltaApplier`. **No-op flag pending PIP-9 wire-up:** PIP-7 (#4730) proved the previous dispatch scaffolding was a side-effect-only no-op that the production receiver never consumed; PIP-8 tore it out so the feature flag can no longer spawn an unobserved `DeltaConsumer` background thread that was the suspected source of the receiver corruption. The `ParallelDeltaApplier` and `ParallelDeltaPipeline` types stay compiled and are exercised by benches (`parallel_receive_delta_perf`, `br_3j_f_dashmap_cores_vs_throughput`, `parallel_verify_chunk`) and fuzz tests. The eventual RJN-3 fan-out integration is tracked by PIP-9 (`docs/design/pip-9-parallel-receive-delta-wire-up-2026-05-22.md`). | experimental (no-op pending PIP-9) |
