...
| `parallel-receive-delta` | `engine`, `transfer` | yes | Rayon-dispatched per-file receive-side delta application via `ParallelDeltaApplier`. **Default-on with one remaining GA-blocking gap** that is not correctness-breaking for the COPY/LITERAL stream itself: the outer `Mutex<HashMap<FileNdx, _>>` serialises every per-chunk slot lookup under high concurrent-file fan-out (BR-3j.a-f), capping throughput well below the rayon-parallel ideal. The earlier verify-chunk scaffold (BR-3i.a-f) was replaced in BR-3i.c with real strong-checksum verification driven by the negotiated [`ChecksumStrategy`] (MD5/MD4/XXH3); mismatches surface as a typed `ParallelApplyError::ChecksumMismatch` so the receiver can fall back to phase-2 redo without committing corrupt bytes. See `docs/design/parallel-receive-delta-application.md`. | stable (with caveats) |
