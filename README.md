...
| `parallel-receive-delta` | `engine`, `transfer` | no | Rayon-dispatched per-file receive-side delta application via `ParallelDeltaApplier`. **DEFERRED pending PIP-7 fix:** default-on flip reverted after the PIP-4 interop scenario surfaced receiver-side corruption when the file count crosses the parallel-dispatch threshold (#4720 follow-up). Opt-in only via `--features parallel-receive-delta`. The earlier verify-chunk scaffold (BR-3i.a-f) was replaced in BR-3i.c with real strong-checksum verification driven by the negotiated [`ChecksumStrategy`] (MD5/MD4/XXH3); mismatches surface as a typed `ParallelApplyError::ChecksumMismatch` so the receiver can fall back to phase-2 redo without committing corrupt bytes. See `docs/design/parallel-receive-delta-application.md`. | experimental (deferred) |
