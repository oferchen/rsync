...
| Workspace | CLI front-end (`bin/rsync`) | Partial | The binary exists and serves `--help`/`--version`. Transfer attempts exit with code `1` because the engine and option parser are not yet wired up; diagnostics are emitted via `core::client::run_client`. | `crates/cli`, `bin/rsync`, `crates/core/src/client.rs` |
...
| Workspace | Core transfer orchestration plus engine/meta/filter/compress crates | Missing | `core::client::run_client` now exists and centralises the user-facing diagnostic, but the transfer engine, metadata (`meta`), filtering, and compression crates are not implemented, leaving delta transfer and metadata application unavailable. | `crates/core/src/client.rs` |
