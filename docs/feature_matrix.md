...
| Workspace | Core transfer orchestration plus engine/meta/filter/compress crates | Missing | `core::client::run_client` provides a deterministic local filesystem copy pipeline. The new `rsync_meta` crate centralises permission and timestamp preservation, but the delta-transfer engine along with ownership/xattr/ACL handling, filtering, and compression crates are still absent. Remote transports and protocol orchestration remain pending. | `crates/core/src/client.rs`, `crates/meta/src/lib.rs` |
