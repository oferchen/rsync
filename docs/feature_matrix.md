...
| Workspace | CLI front-end (`bin/oc-rsync`) | Partial | The binary supports `--help`, `--version`, `--dry-run`, `--archive`/`-a` (implying `--owner`/`--group`), `--delete`, `--exclude`, `--include`, `--filter` with `+`/`-` actions, `--owner`, `--group`, `--numeric-ids`/`--no-numeric-ids`, and `--bwlimit`, performs deterministic local copies for regular files, directories, symbolic links, and FIFOs while preserving permissions, timestamps, and optional ownership metadata, and can list modules from `rsync://` daemons using the legacy handshake. Remote transfers, xattrs/ACLs, comprehensive filter directives, and compression remain unimplemented. | `crates/cli`, `bin/oc-rsync`, `crates/core/src/client.rs` |
