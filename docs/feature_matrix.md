...
| Workspace | CLI front-end (`bin/oc-rsync`) | Partial | The binary supports `--help`, `--version`, and deterministic local copies for regular files and directories. Remote transports, metadata preservation, filters, and compression remain unimplemented. | `crates/cli`, `bin/oc-rsync`, `crates/core/src/client.rs` |
...
| Workspace | Core transfer orchestration plus engine/meta/filter/compress crates | Missing | `core::client::run_client` provides a deterministic local filesystem copy pipeline, but the delta-transfer engine, metadata (`meta`), filtering, and compression crates are not implemented. Remote transports and protocol orchestration are pending. | `crates/core/src/client.rs` |
