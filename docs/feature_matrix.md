...
| Core | Version metadata and standard banner formatting | Implemented | `version_metadata()` exposes upstream constants and renders the canonical `--version` banner (`oc-rsync  version 3.4.1-rust (revision/build #REV)  protocol version 32`, copyright notice, and web site). | `crates/core/src/version.rs` |
...
| Workspace | CLI front-end (`bin/oc-rsync`) | Partial | The binary supports `--help`, `--version`, `--dry-run`, deterministic local copies for regular files, directories, and symbolic links while preserving permissions and modification times, and can list modules from `rsync://` daemons using the legacy handshake. Remote transfers, ownership/xattrs/ACLs, filters, and compression remain unimplemented. | `crates/cli`, `bin/oc-rsync`, `crates/core/src/client.rs` |
...
| Workspace | Daemon server (`bin/oc-rsyncd`) | Partial | Listens on a configurable TCP socket, completes the legacy handshake for sequential connections, serves `#list` requests using modules provided via `--module` or `--config` (subset of `rsyncd.conf`), and returns a deterministic `@ERROR` response while full module serving is pending. | `crates/daemon/src/lib.rs` |
| Workspace | Core transfer orchestration plus engine/meta/filter/compress crates | Partial | `core::client::run_client` now delegates to [`LocalCopyPlan`](../crates/engine/src/local_copy.rs) for deterministic local filesystem copies preserving permissions and timestamps. Delta-transfer logic, ownership/xattr/ACL handling, filtering, compression, and remote transport orchestration remain pending. | `crates/core/src/client.rs`, `crates/engine/src/local_copy.rs`, `crates/meta/src/lib.rs` |
