...
1. **FFI floor.** Land `crates/fast_io/src/dispatch_io/ffi.rs` plus a
   ...
   five crates listed in <internal-tool-doc>'s "Unsafe Code Policy" as permitted to
...
- **Block ABI surface area.** The block-pointer ABI for io_handler and
  ...
  <internal-tool-doc>'s "Standard library first" rule weighs against `block2`; the
...
- **`dispatch_data_t` lifetime vs `BufferPool`.** The most attractive design
  ...
  `Mutex<Vec<Vec<u8>>>` (per <internal-tool-doc> "Buffer pool contention" note), and
...
- **Benchmark gap.** The benchmark scripts (`scripts/benchmark.sh`,
  ...
  `localhost/oc-rsync-bench:latest` Linux container per <internal-tool-doc>
