...
1. Per-flag emission gating is absent for COPY, DEL, FLIST, MISC,
   MOUNT, REMOVE, SKIP, SYMSAFE. BACKUP is now wired through
   `backup_existing_entry` in
   `crates/engine/src/local_copy/context_impl/state.rs`, and NONREG is
   wired through `record_skipped_non_regular` in
   `crates/engine/src/local_copy/context_impl/reporting.rs`.
   ...
   `crates/engine/src/local_copy/*` (COPY),
