...
| --------- | ------------------ | ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| BACKUP    | 1 (W_REC)          | Mention files backed up                                            | `crates/logging/src/levels/info.rs:54-138` (`InfoLevels::backup`); `crates/cli/src/frontend/execution/flags/info.rs:112-115` accepts arbitrary level (no upper cap, see `parse_flag_and_level`); `crates/cli/src/frontend/info_output.rs:325` parses identically.                                                                       | None in production. `should_show_backup` exists at `crates/cli/src/frontend/info_output.rs:154-156` but no caller in `crates/{engine,core,transfer,daemon,filters,metadata}/src` issues an `info_gte`/`should_show_backup` gate before emitting backup messages.                                                                          | Stub        |
| COPY      | 1 (W_REC)          | Mention files copied locally on the receiving side                 | Same path as BACKUP; `crates/cli/src/frontend/execution/flags/info.rs:116-119` accepts any level (no cap).                                                                                                                                                                                                                            | None in production. `should_show_copy` at `crates/cli/src/frontend/info_output.rs:162-164` is unused outside tests.                                                                                                                                                                                                                          | Stub        |
...
| MOUNT     | 1 (W_SND\|W_REC)   | Mention mounts that were found or skipped                          | `crates/cli/src/frontend/execution/flags/info.rs:138-141`, `info_output.rs:330`. No upper cap.                                                                                                                                                                                                                                       | None in production. Upstream uses `INFO_GTE(MOUNT, 1)` in `flist.c:1319` and `generator.c:325`. Our `--one-file-system` traversal does not log mount-skip events under any info gate.                                                                                                                                                       | Stub        |
...
| NONREG    | 1 (W_REC, default 1; 0 disables) | Mention skipped non-regular files (default 1, 0 disables)           | `crates/cli/src/frontend/execution/flags/info.rs:142-145`, `info_output.rs:332`. Upstream's "default 1" semantics live in `info_verbosity[0]`; we set `nonreg = 1` for all `-v` levels including 0 at `crates/logging/src/config.rs:49,53,65,..`.                                                                                          | None in production. Upstream `INFO_GTE(NONREG, 1)` fires in `backup.c:307` and `generator.c:1684`. We do not emit a "skipping non-regular file" message anywhere; the gate exists in parsing but no producer.                                                                                                                                | Stub        |
...
1. Per-flag emission gating is absent for BACKUP, COPY, DEL, FLIST, MISC,
   MOUNT, NONREG, REMOVE, SKIP, SYMSAFE. Adding `info_gte(InfoFlag::*, n)`
   guards at the upstream call sites listed above is required to graduate
   each flag from "stub" or "parsed-only" to "match". Suggested insertion
   crates: `crates/engine/src/generator/*` (FLIST, SKIP, NONREG),
   `crates/transfer/src/file_list/*` (FLIST, MOUNT, SYMSAFE),
   `crates/engine/src/local_copy/*` (BACKUP, COPY),
