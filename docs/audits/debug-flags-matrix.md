...
| IO       | 4            | 4            | CLI+SRV   | Debug I/O routines (levels 1-4)                        | `protocol/src/debug_io.rs`, target `rsync::io`. `rsync_io/src/debug_io.rs` never existed; `fast_io/src/debug_io.rs` was reachable from no `mod` or `include!` and so was never compiled - removed | Wired |
