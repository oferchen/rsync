...
documented advantage" (<internal-tool-doc>, Code Quality section) for synchronisation
...
- Hot path? **Borderline.** The connection pool is touched once per
  ...
  transfer.
- Note: the policy in <internal-tool-doc> is about `std::sync::Mutex` /
...
- Policy source: `<internal-tool-doc>` "Code Quality" section ("prefer std unless a
