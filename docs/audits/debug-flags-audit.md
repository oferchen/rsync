...
| IO         | 4            | 4       | CLI+SRV    | Debug I/O routines (levels 1-4)                        | `crates/protocol/src/debug_io.rs` (target `rsync::io`). Two further modules were listed here and neither existed: `crates/rsync_io/src/debug_io.rs` was never in the tree, and `crates/fast_io/src/debug_io.rs` was reachable from no `mod` or `include!`, so the compiler never parsed it - it has been removed.                                                                                                                | Yes |
