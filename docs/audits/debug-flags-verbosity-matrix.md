...
- **impl**: 13 - ACL, BACKUP, CMD, DUP, FILTER, FLIST, GENR, HASH, ICONV, NSTR, OWN, SEND, TIME.
- **partial**: 7 - CONNECT, DEL, DELTASUM, EXIT, IO, PROTO, RECV.
- **missing**: 4 - BIND, CHDIR, FUZZY, HLINK.
...
| G3 | High | open | 4 flags have no production producer (BIND, CHDIR, FUZZY, HLINK). The previously listed ACL, BACKUP, CMD, GENR, HASH, ICONV, NSTR, OWN, and SEND emissions are now wired (D14, I-ACL, I-ICONV, NSTR follow-up, OWN follow-up, D13 RESOLVED, BACKUP RESOLVED, HASH RESOLVED, CMD RESOLVED). Owning crates for the remaining producers: `metadata` (HLINK), `engine` (FUZZY), `rsync_io` (BIND), `core` (CHDIR). |
