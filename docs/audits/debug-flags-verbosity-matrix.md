...
| BIND       | 1 | u8::MAX | W_CLI | Debug socket bind actions | `crates/daemon/src/daemon/sections/server_runtime/listener.rs::bind_with_backlog` (2 sites at level 1: `trace_socket_failure` after a failed `socket(2)` and `trace_bind_failure` after a failed `bind(2)`). Helpers live in `crates/protocol/src/bind/trace.rs` and mirror upstream `socket.c:432-470` byte-for-byte. | impl (G3 partial - BIND RESOLVED) |
| CHDIR      | 1 | u8::MAX | W_CLI\|W_SRV | Debug when the current directory changes | `crates/daemon/src/daemon/sections/privilege.rs::apply_chroot` (1 site at level 1) invokes `protocol::chdir::trace_change_dir(ChdirRole::PreForkReceiver, "/")` immediately after the platform-level `chroot` + `chdir("/")` succeeds, mirroring upstream `clientserver.c:987` `change_dir(module_chdir, CD_NORMAL)`. Helper lives in `crates/protocol/src/chdir/trace.rs` and emits upstream-verbatim wording from `util1.c:1168-1169` (`"[%s] change_dir(%s)\n"`); the `ChdirRole` enum mirrors the `who_am_i()` tokens from `rsync.c:823-830`. | impl (G3 partial - CHDIR RESOLVED) |
| CMD        | 2 | 2 | W_CLI | Debug commands+options that are issued (levels 1-2) | `crates/rsync_io/src/ssh/builder.rs::SshCommand::spawn` (level 1 opening-connection + level 2 enumeration), `crates/core/src/client/remote/daemon_transfer/orchestration/arguments.rs::send_daemon_arguments` (level 1 sending-daemon-args, plus level 1 protected-args when `--protect-args` is active), `crates/core/src/client/remote/ssh_transfer.rs::build_ssh_connection` (level 1 protected-args for SSH stdin secluded-args). Helpers live in `crates/protocol/src/cmd/trace.rs` and mirror upstream `util1.c:98-117`, `pipe.c:54-55`, `clientserver.c:348-349`, `rsync.c:296-297`, and `main.c:620-624`. | impl (G3 partial - CMD RESOLVED) |
...
- **impl**: 15 - ACL, BACKUP, BIND, CHDIR, CMD, DUP, FILTER, FLIST, GENR, HASH, ICONV, NSTR, OWN, SEND, TIME.
- **partial**: 7 - CONNECT, DEL, DELTASUM, EXIT, IO, PROTO, RECV.
- **missing**: 2 - FUZZY, HLINK.
...
| BIND | wired in `crates/daemon/src/daemon/sections/server_runtime/listener.rs::bind_with_backlog` (level 1 `trace_socket_failure` after a failed `socket(2)` and `trace_bind_failure` after a failed `bind(2)`, called from the per-address-family loop in `accept_loop.rs`). Helpers live in `crates/protocol/src/bind/trace.rs` and mirror upstream `socket.c:432-470` byte-for-byte: `"socket(%d,%d,%d) failed: %s"` and `"bind() failed: %s (address-family %d)"`. The upstream `socket.c:479-486` flush loop semantics are preserved - per-family error messages fire only when `--debug=BIND` is at level 1, while the all-bound-failed `FERROR` line at `socket.c:488-491` is handled separately by `bind_error()` in `accept_loop.rs` and is not gated by the debug flag. |
| CHDIR | wired in `crates/daemon/src/daemon/sections/privilege.rs::apply_chroot` (level 1) invoking `protocol::chdir::trace_change_dir(ChdirRole::PreForkReceiver, "/")` immediately after `platform::privilege::apply_chroot` returns `Ok` (post `chroot` + `chdir("/")`). Helper lives in `crates/protocol/src/chdir/trace.rs` and mirrors upstream `util1.c:1168-1169` (`"[%s] change_dir(%s)\n"`); the `ChdirRole` enum mirrors `who_am_i()` (`rsync.c:823-830`) so additional callsites (oc-rsync currently has none - the sender resolves paths logically without `chdir(2)`, matching the design notes in `crates/transfer/src/generator/file_list/mod.rs:135-140`) can plug in directly. Upstream emits at the single `change_dir` site in `util1.c:1113-1172`, gated by `!set_path_only`; only the daemon chroot path executes the syscall in oc-rsync. |
| CMD | wired in `crates/rsync_io/src/ssh/builder.rs::SshCommand::spawn` (level 1 opening-connection + level 2 `cmd[i]=value` enumeration immediately before `Command::spawn`), `crates/core/src/client/remote/daemon_transfer/orchestration/arguments.rs::send_daemon_arguments` (level 1 sending-daemon-args before the per-arg socket write loop, plus level 1 protected-args before the secluded-args send when `--protect-args` is active), and `crates/core/src/client/remote/ssh_transfer.rs::build_ssh_connection` (level 1 protected-args before the SSH stdin secluded-args send). Helpers live in `crates/protocol/src/cmd/trace.rs` and mirror upstream `util1.c:98-117` (`print_child_argv` quoting and `(N args)` suffix), `pipe.c:54-55`, `clientserver.c:348-349`, `rsync.c:296-297`, and `main.c:620-624`. The level-2 `cmd=`/`machine=`/`user=`/`path=` line from `main.c:1564` has no oc-rsync counterpart because the equivalent `do_cmd`-style shell parsing is folded into `RemoteShell::build_command` and `SshCommand`; the rest of the upstream CMD surface is covered. |
...
| NSTR | wired in `crates/protocol/src/negotiation/capabilities/negotiate.rs::negotiate_capabilities_with_override` (levels 1-3 via `trace_send_list`, `trace_recv_list`, `trace_checksum_summary`, and `trace_compress_summary`) and `crates/core/src/client/remote/daemon_transfer/connection/mod.rs::perform_daemon_handshake` (level 1 `trace_daemon_auth_negotiated` after `select_daemon_digest`, level 2 `trace_daemon_greeting_auth_list` after parsing the greeting's `@RSYNCD: %d.%d %s` banner). Helpers live in `crates/protocol/src/nstr/trace.rs` and mirror upstream `compat.c:213-219,373-378,521-525,843-844,865-868` and `checksum.c:206-211` byte-for-byte. The conditional `" negotiated"` qualifier follows upstream's `valid_*.negotiated_nni != NULL` branch (compat.c:217/checksum.c:209): it renders when negotiation went through `negotiate_the_strings()` and stays blank when the user forced via `--checksum-choice` / `--compress-choice`. The compress summary always renders `(level <N>)` and passes `CLVL_NOT_SPECIFIED == INT_MIN` when `--compress-level` is unset (compat.c:218, rsync.h:1151). |
