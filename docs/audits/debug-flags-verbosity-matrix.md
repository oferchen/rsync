...
| G3 | High | open | 2 flags have no production producer (FUZZY, HLINK). The previously listed ACL, BACKUP, BIND, CHDIR, CMD, GENR, HASH, ICONV, NSTR, OWN, and SEND emissions are now wired (D14, I-ACL, I-ICONV, NSTR follow-up, OWN follow-up, D13 RESOLVED, BACKUP RESOLVED, HASH RESOLVED, CMD RESOLVED, CHDIR RESOLVED, BIND RESOLVED). Owning crates for the remaining producers: `metadata` (HLINK), `engine` (FUZZY). |
