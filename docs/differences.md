...
the branded name **oc-rsync 3.4.1-rust** as declared in `Cargo.toml`. The
workspace ships a single binary (`oc-rsync`) that covers both client and
daemon roles. Downstream packages that require alternate names must provide
their own symlinks without relying on additional binaries from this
repository. Items
...
- **`oc-rsync` client uses native local copies and a fallback for remote transfers**
  - *Impact*: `oc-rsync` performs deterministic local filesystem copies for
    ...
    and `merge`/`dir-merge` directives (including their `.`/`:` shorthands)
    mirrors rsync's glob semantics for local copies. Progress
...
- **Transfer engine integration incomplete**
  - *Impact*: The `oc_rsync_engine` crate provides deterministic local copies for
