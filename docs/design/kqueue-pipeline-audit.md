...
| Daemon accept (single listen) | Migrated (KQ-S.1): `KqueueAcceptEngine` registers a level-triggered `EVFILT_READ` per listener on `KqueueLoop` and parks in `kevent(2)` with a 100 ms signal-check timeout (PR #6217, opt-in `macos-kqueue` feature). Without the feature, `PollAcceptEngine` parks in one `poll(2)` over every listener fd with a 50 ms signal-check bound (PR #7681). The former `thread::sleep(SIGNAL_CHECK_INTERVAL)` accept polling is gone. | `crates/daemon/src/daemon/sections/server_runtime/accept_engine.rs`               | n/a             | Closed for the wait primitive: both engines are readiness-driven. Remaining gap is only that the kqueue engine sits behind the default-off `macos-kqueue` feature (default-flip gated by KQ-7 bench numbers per KQ-8).    |
| Daemon accept (dual stack)    | Migrated: the per-listener acceptor threads and MPSC fan-in were removed by PR #7681; one engine polls every bound listener fd from the accept thread (the kqueue engine registers each fd on the same `KqueueLoop`).             | `crates/daemon/src/daemon/sections/server_runtime/accept_engine.rs`               | n/a             | Closed: no acceptor-thread fan-out remains to replace.                                                                                                                                                                    |
