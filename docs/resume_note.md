...
## Status
- Workspace implements the canonical **oc-rsync 3.4.1-rust** entrypoint (client plus `--daemon`) with a deterministic local copy engine, message formatting, and protocol negotiation scaffolding that already mirrors upstream interfaces for local execution. Deployments that still require alternate names can add compatibility symlinks, but the workspace only builds the oc-prefixed binary to avoid collisions.
