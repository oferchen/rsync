...
- **Daemon orchestration (handshake only)**
  - *Impact*: `oc-rsyncd` binds a TCP listener, performs the legacy
    `@RSYNCD:` negotiation, and returns a deterministic `@ERROR` response. Real
    module configuration, authentication, and file serving remain unavailable.
