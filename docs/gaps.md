...
> `[workspace.metadata.oc_rsync]`. Optional wrappers (`oc-rsyncd`, `rsync`,
> `rsyncd`) remain available behind the `legacy-binaries` feature and reuse the
> single-binary entrypoint so existing tooling can opt into alternate naming
> explicitly without affecting default packages.
...
- **Signature generation** (`crates/engine::signature`):
  ...
  rolling and strong checksum primitives from `oc_rsync_checksums` to emit
...
- **Client transfer orchestration (local filesystem + daemon module listing)**
  - *Impact*: `oc-rsync` supports copying regular files, directory trees,
    ...
    deletion family (`--delete*`, `--delete-missing-args`, `--delete-excluded`, `--remove-source-files`),
...
- **CI packaging and upstream interoperability harness**
  - *Impact*: Release automation builds `.deb`, `.rpm`, and tarball artifacts
    ...
    `oc-rsync` under dedicated paths so upstream packages can remain on the
    system without shipping extra wrapper binaries. The
    ...
    oc-rsync --daemon and oc-rsync ↔ upstream daemon—to confirm protocol compatibility
  ...
  - *Removal plan*: Extend the harness with the exit-code oracle, golden tree
    ...
    fully automated.
- **Parity harness and golden comparisons**
  - *Impact*: A deterministic golden harness captures the expected
    `oc-rsync --version` output under controlled locale/time/terminal settings
    and validates the binary against the upstream text snapshot as part of the
    integration tests. Additional CLI/daemon scenarios, exit-code oracles,
    tree-hash comparisons, and SSH transport coverage have not landed yet.
  - *Removal plan*: Extend the harness with further scenarios, exit-code
    validation, filesystem tree comparisons, and SSH transport coverage so the
    parity matrix becomes fully automated.

## Missing
- **Transfer engine integration with transports**: the
  [`oc_rsync_engine`](../crates/engine/src/delta/) crate emits rsync-compatible
