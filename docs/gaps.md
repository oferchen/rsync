...
> `oc-rsync` (client) and `oc-rsyncd` (daemon) as defined in
> `[workspace.metadata.oc_rsync]`. Compatibility wrappers (`rsync`,
> `rsyncd`) remain available and share the same execution paths so existing
> tooling can opt into the legacy branding explicitly.
...
- **CI packaging and upstream interoperability harness**
  - *Impact*: Release automation builds `.deb`, `.rpm`, and tarball artifacts
    ...
    `oc-rsync`/`oc-rsyncd` under dedicated paths so upstream packages can
    remain on the system. The `tools/ci/run_interop.sh` harness now downloads
    upstream rsync releases 3.0.9, 3.1.3, and 3.4.1 and exercises both
    directions—upstream client ↔ oc-rsyncd and oc-rsync ↔ upstream daemon—to
    confirm protocol compatibility while delegation remains in place.
