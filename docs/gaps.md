...
- **Client orchestration and local copy pipeline** (`crates/core::client`):
  [`run_client`](../crates/core/src/client.rs) performs deterministic local
  filesystem copies for regular files and directories, providing the foundation
  that future transfer-engine integrations will extend.
...
## Partial
- **Client transfer orchestration (local filesystem only)**
  - *Impact*: `oc-rsync` supports copying regular files, directory trees, and
    symbolic links on the local filesystem. Remote transports, delta compression, metadata
    preservation, filters, bandwidth limits, and progress reporting are not yet
    implemented.
...
- **Daemon orchestration**
  - *Impact*: The `rsyncd` binary emits a diagnostic explaining that server mode
    is unavailable, so daemon sessions and module hosting remain impossible.
  - *Removal plan*: Implement the daemon crate's transport loop, configuration
    handling, and module orchestration, then integrate it with the `core`
    facade so real daemon sessions execute.
- **Transfer engine and supporting crates**: the `engine`, `filters`, and
  `compress` crates remain unimplemented. The new
  [`rsync_meta`](../crates/meta/src/lib.rs) crate centralises permission and
  timestamp preservation, but ownership, xattrs, ACLs, and sparse handling are
  still missing. Consequently, [`core::client::run_client`](../crates/core/src/client.rs)
  only performs best-effort local copies; delta transfer and remote transports
  remain unavailable.
