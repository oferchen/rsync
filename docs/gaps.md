...
- **Feature-gated metadata, compression, and logging integration**: optional
  ...
  [`CountingZlibEncoder`](../crates/compress/src/zlib.rs) and
...
- **Transfer engine integration with transports**: the
  ...
  [`CountingZlibEncoder`](../crates/engine/src/local_copy.rs) and, when the
  `zstd` feature is enabled, [`CountingZstdEncoder`](../crates/engine/src/local_copy/compressor.rs).
  The pipeline is
