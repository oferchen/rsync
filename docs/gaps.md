...
> `[workspace.metadata.oc_rsync]`. Deployments that still rely on alternate
> names can provide symlinks on their own, but the workspace builds and ships
> only the oc-prefixed binary by default.
