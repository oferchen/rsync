...
- **Client transfer orchestration (local filesystem + daemon module listing)**
  - *Impact*: `oc-rsync` supports copying regular files, directory trees,
    ...
    `rsync://` daemons. Remote transports, delta compression, extended
    metadata such as xattrs/ACLs, advanced filter directives beyond
    `--filter` `+`/`-` actions, and progress reporting are not yet implemented.
...
- **Transfer engine extensions and supporting crates**: the
  ...
  local copy planner used by `oc-rsync`, but delta transfer, filters, and
  compression remain unimplemented. Metadata features such as xattrs and ACLs
  are still pending in [`rsync_meta`](../crates/meta/src/lib.rs).
