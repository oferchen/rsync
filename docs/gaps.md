...
- **Client transfer orchestration (local filesystem + daemon module listing)**
  - *Impact*: `oc-rsync` supports copying regular files, directory trees, and
    symbolic links on the local filesystem, provides a `--dry-run` validation
    mode, and can list modules from `rsync://` daemons. Remote transports,
    delta compression, metadata preservation, filters, bandwidth limits, and
