...
- **Client transfer orchestration (local filesystem + daemon module listing)**
  - *Impact*: `oc-rsync` supports copying regular files, directory trees,
    ...
    `protect PATTERN`, `exclude-if-present=FILE`, and `merge FILE`
...
- **Transfer engine extensions and supporting crates**: the
  [`rsync_engine`](../crates/engine/src/delta/) crate now produces file
  signatures and can derive rsync-style delta token streams (see
  [`DeltaGenerator`](../crates/engine/src/delta/generator.rs)) together with a
  reusable [`DeltaSignatureIndex`](../crates/engine/src/delta/index.rs) and
  [`apply_delta`](../crates/engine/src/delta/script.rs) helper. The pipeline is
  still local-only: compression, full filter semantics, and transport
  orchestration remain unimplemented, and the client/daemon binaries continue to
  rely on the system `rsync` for network transfers until those components land.
- **Feature gating and runtime integration**: compression, metadata
