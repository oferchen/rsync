...
> `[workspace.metadata.oc_rsync]`. Alternate binary names are not provided by
> default; downstream packaging may add symlinks if required, but the
> workspace itself only builds the unified `oc-rsync` binary.
