...
The generated YAML also names the oc-rsync extensions so downstream tooling and documentation can track deliberate divergences from rsync 3.4.1. 【F:docs/parity-options.yml†L1508-L1520】

| option | rationale | notes |
| ------ | --------- | ----- |
| `--connect-program=COMMAND` | Allows operators to inject a daemon transport helper when talking to `rsync://` endpoints; clap captures the value and the fallback runner forwards it to the upstream binary even though rsync 3.4.1 lacks this entry. 【F:crates/cli/src/frontend/command_builder/sections/build_base_command.rs†L92-L102】【F:crates/core/src/client/fallback/runner/command_builder.rs†L633-L665】【F:docs/parity-options.yml†L1508-L1511】 |
| `--no-copy-links`, `--no-copy-unsafe-links`, `--no-keep-dirlinks` | oc-rsync exposes explicit negations for symlink-handling toggles so scripts can disable behaviours without relying on positional ordering; upstream only documents the positive forms. 【F:crates/cli/src/frontend/command_builder/sections/build_base_command.rs†L315-L385】【F:docs/parity-options.yml†L1512-L1520】 |
