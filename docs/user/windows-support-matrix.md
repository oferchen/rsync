...
| Symbolic links (`-l`) | Shipped (directory links fall back to junctions when unprivileged) | `crates/fast_io/src/win_symlink.rs` (`create_directory_symlink_or_junction` / `create_file_symlink`, called from `crates/engine/src/local_copy/executor/special/symlink.rs::create_symlink`). Directory links try `CreateSymbolicLinkW` with `SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE` and fall back to a junction (`FSCTL_SET_REPARSE_POINT`) on `ERROR_PRIVILEGE_NOT_HELD`; file links have no fallback and are skipped with a warning and a soft (exit 23) error. Coverage: `crates/fast_io/src/win_symlink.rs` tests + `crates/engine/src/local_copy/tests/execute_symlink_edge_cases.rs` (`windows_directory_symlink_falls_back_to_junction`, `windows_file_symlink_reports_privilege_gracefully`); end-to-end push round-trip at `crates/metadata/tests/windows_symlink_junction_transfer.rs`. |
