...
- **No delegation to system `rsync`**
  All entry points (client, server, daemon) must execute natively inside the
  Rust binary. Historical fallbacks such as `OC_RSYNC_FALLBACK` are ignored in
  favour of clear diagnostics when an unimplemented role is requested.
