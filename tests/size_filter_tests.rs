...
fn max_size_with_kilobyte_suffix() {
    ...
    assert!(dest_dir.join("under_2k.txt").exists(), "File under 2K should be transferred");
    assert!(dest_dir.join("at_2k.txt").exists(), "File at exactly 2K should be transferred");
    assert!(!dest_dir.join("over_2k.txt").exists(), "File over 2K should be excluded");
}
...
fn min_size_with_kilobyte_suffix() {
    ...
    assert!(!dest_dir.join("under_1k.txt").exists(), "File under 1K should be excluded");
    assert!(dest_dir.join("at_1k.txt").exists(), "File at exactly 1K should be transferred");
    assert!(dest_dir.join("over_1k.txt").exists(), "File over 1K should be transferred");
}
...
fn max_size_with_megabyte_suffix() {
    ...
    assert!(dest_dir.join("under_1m.txt").exists(), "File under 1M should be transferred");
    assert!(dest_dir.join("at_1m.txt").exists(), "File at exactly 1M should be transferred");
    assert!(!dest_dir.join("over_1m.txt").exists(), "File over 1M should be excluded");
}
...
fn min_size_with_megabyte_suffix() {
    ...
    assert!(!dest_dir.join("under_1m.txt").exists(), "File under 1M should be excluded");
    assert!(dest_dir.join("at_1m.txt").exists(), "File at exactly 1M should be transferred");
    assert!(dest_dir.join("over_1m.txt").exists(), "File over 1M should be transferred");
}
...
fn max_size_with_gigabyte_suffix_includes_small_files() {
    ...
    assert!(dest_dir.join("small.txt").exists(), "Small file should be transferred");
    assert!(dest_dir.join("medium.txt").exists(), "Medium file should be transferred");
}
...
fn min_size_with_gigabyte_suffix_excludes_small_files() {
    ...
    assert!(!dest_dir.join("small.txt").exists(), "Small file should be excluded");
    assert!(!dest_dir.join("medium.txt").exists(), "Medium file should be excluded");
}
...
fn size_suffix_case_insensitive() {
    ...
    assert!(!dest_dir.join("file.txt").exists(), "File should be excluded with lowercase suffix");
}
...
fn max_size_fractional_value() {
    ...
    assert!(dest_dir.join("at_limit.txt").exists(), "File at 1.5K should be transferred");
    assert!(!dest_dir.join("over_limit.txt").exists(), "File over 1.5K should be excluded");
}
...
fn combined_max_and_min_size_filters() {
    ...
    assert!(!dest_dir.join("too_small.txt").exists(), "File under min-size should be excluded");
    assert!(dest_dir.join("at_min.txt").exists(), "File at min-size should be included");
    assert!(dest_dir.join("in_range.txt").exists(), "File in range should be included");
    assert!(dest_dir.join("at_max.txt").exists(), "File at max-size should be included");
    assert!(!dest_dir.join("too_large.txt").exists(), "File over max-size should be excluded");
}
...
fn combined_filters_with_units() {
    ...
    assert!(!dest_dir.join("too_small.txt").exists(), "File under 1K should be excluded");
    assert!(dest_dir.join("in_range.txt").exists(), "File between 1K-10K should be included");
    assert!(!dest_dir.join("too_large.txt").exists(), "File over 10K should be excluded");
}
...
fn max_size_includes_empty_files() {
    ...
    assert!(dest_file.exists(), "Empty file should be transferred (0 < max-size)");
    assert_eq!(fs::metadata(&dest_file).unwrap().len(), 0, "File should be empty");
}
...
fn min_size_excludes_empty_files() {
    ...
    assert!(!dest_file.exists(), "Empty file should be excluded (0 < min-size)");
}
...
fn min_size_zero_includes_empty_files() {
    ...
    assert!(dest_dir.join("empty.txt").exists(), "Empty file should be included with min-size=0");
    assert!(dest_dir.join("nonempty.txt").exists(), "Non-empty file should be included");
}
...
fn size_filters_do_not_affect_directories() {
    ...
    assert!(dest_dir.join("subdir/nested").is_dir(), "Nested dir should be created");

    // Small file should be excluded, large file should exist
    assert!(!dest_dir.join("subdir/small.txt").exists(), "Small file should be excluded");
    assert!(dest_dir.join("subdir/nested/large.txt").exists(), "Large file should be transferred");
}
...
fn max_size_with_dry_run() {
    ...
    assert!(!dest_dir.join("small.txt").exists(), "Dry run should not create files");
    assert!(!dest_dir.join("large.txt").exists(), "Dry run should not create files");
}
...
fn max_size_with_archive_mode() {
    ...
    assert!(dest_dir.join("small.txt").exists(), "Small file should be transferred");
    assert!(!dest_dir.join("large.txt").exists(), "Large file should be excluded");
}
...
fn min_size_with_archive_mode() {
    ...
    assert!(!dest_dir.join("small.txt").exists(), "Small file should be excluded");
    assert!(dest_dir.join("large.txt").exists(), "Large file should be transferred");
}
...
fn max_size_decimal_kilobyte_suffix() {
    ...
    assert!(dest_dir.join("under_1kb.txt").exists(), "File under 1KB should be transferred");
    assert!(dest_dir.join("at_1kb.txt").exists(), "File at exactly 1KB should be transferred");
    assert!(!dest_dir.join("over_1kb.txt").exists(), "File over 1KB should be excluded");
}
...
fn max_size_explicit_binary_suffix() {
    ...
    assert!(dest_dir.join("at_1k.txt").exists(), "File at exactly 1KiB should be transferred");
    assert!(!dest_dir.join("over_1k.txt").exists(), "File over 1KiB should be excluded");
}
...
fn max_size_does_not_delete_existing_dest_files() {
    ...
    assert!(dest_dir.join("large.txt").exists(), "Existing dest file should remain");
    let content = fs::read(dest_dir.join("large.txt")).unwrap();
    assert_eq!(content, b"existing content", "Dest file content should be unchanged");
}
...
fn size_filter_incremental_transfer() {
    ...
    assert!(dest_dir.join("file.txt").exists(), "First transfer should succeed");
